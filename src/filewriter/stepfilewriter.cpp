...
void StepFileWriter::WriteProductHeader() {
  fprintf(f, "#175 = SHAPE_DEFINITION_REPRESENTATION(#176, #169);\n"
...
int StepFileWriter::ExportCurve(SBezier *sb) {
  ...
  fprintf(f, "B_SPLINE_CURVE_WITH_KNOTS((%d,%d),", (sb->deg + 1), (sb->deg + 1));
...
int StepFileWriter::ExportCurveLoop(SBezierLoop *loop, bool inner) {
  ...
  for (oe = listOfTrims.First(); oe; oe = listOfTrims.NextAfter(oe)) {
    fprintf(f, "#%d", *oe);
    if (listOfTrims.NextAfter(oe) != NULL)
      fprintf(f, ",");
  }
  fprintf(f, "));\n");
...
void StepFileWriter::ExportSurface(SSurface *ss, SBezierList *sbl) {
  ...
  fprintf(f, "B_SPLINE_SURFACE_WITH_KNOTS((%d,%d),(%d,%d),", (ss->degm + 1), (ss->degm + 1),
          (ss->degn + 1), (ss->degn + 1));
  ...
  for (i = 0; i <= ss->degm; i++) {
    for (j = 0; j <= ss->degn; j++) {
      fprintf(f, "#%d=CARTESIAN_POINT('',(%.10f,%.10f,%.10f));\n",
              srfid + 1 + j + i * (ss->degn + 1), CO(ss->ctrl[i][j]));
    }
  }
  fprintf(f, "\n");
  ...
  sblss.FindOuterFacesFrom(sbl, &spxyz, ss, SS.ExportChordTolMm(), &allClosed, &notClosedAt, NULL,
                           NULL, NULL);
  ...
  for (sbls = sblss.l.First(); sbls; sbls = sblss.l.NextAfter(sbls)) {
    ...
    for (fb = listOfLoops.First(); fb; fb = listOfLoops.NextAfter(fb)) {
      fprintf(f, "#%d", *fb);
      if (listOfLoops.NextAfter(fb) != NULL)
        fprintf(f, ",");
    }
    ...
    fprintf(f, "#%d=COLOUR_RGB('',%.2f,%.2f,%.2f);\n", ++id, ss->color.redF(), ss->color.greenF(),
            ss->color.blueF());
...
void StepFileWriter::ExportSurfacesTo(const Platform::Path &filename) {
  ...
  for (af = advancedFaces.First(); af; af = advancedFaces.NextAfter(af)) {
    fprintf(f, "#%d", *af);
    if (advancedFaces.NextAfter(af) != NULL)
      fprintf(f, ",");
  }
  fprintf(f, "));\n");
...
void StepFileWriter::WriteWireframe() {
  ...
  for (c = curves.First(); c; c = curves.NextAfter(c)) {
    fprintf(f, "#%d", *c);
    if (curves.NextAfter(c) != NULL)
      fprintf(f, ",");
  }
  fprintf(f, "));\n");
  fprintf(f,
          "#%d=GEOMETRICALLY_BOUNDED_WIREFRAME_SHAPE_REPRESENTATION"
          "('',(#%d,#170),#168);\n",
          id + 1, id);
  fprintf(f, "#%d=SHAPE_REPRESENTATION_RELATIONSHIP($,$,#169,#%d);\n", id + 2, id + 1);
