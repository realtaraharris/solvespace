...
void PdfFileWriter::StartFile () {
  if ((ptMax.x - ptMin.x) > 200 * 25.4 || (ptMax.y - ptMin.y) > 200 * 25.4) {
...
void PdfFileWriter::Background (RgbaColor color) {
  ...
  fprintf (f,
           ...
           "%.3f %.3f l\r\n"
           "b\r\n",
           color.redF (), color.greenF (), color.blueF (), color.redF (), color.greenF (),
           color.blueF (), MmToPts (sw), MmToPts (0), MmToPts (0), MmToPts (width), MmToPts (0),
           MmToPts (width), MmToPts (height), MmToPts (0), MmToPts (height));
}

void PdfFileWriter::StartPath (RgbaColor strokeRgb, double lineWidth, bool filled,
  ...
  fprintf (f,
           ...
           "%.3f w [%s] 0 d "
           "%.3f %.3f %.3f RG\r\n",
           MmToPts (lineWidth), MakeStipplePattern (pattern, stippleScale, ' ').c_str (),
...
void PdfFileWriter::FinishPath (RgbaColor strokeRgb, double lineWidth, bool filled,
                                RgbaColor fillRgb, hStyle hs) {
  if (filled) {
    fprintf (f, "b\r\n");
  } else {
...
void PdfFileWriter::Triangle (STriangle *tr) {
  ...
  fprintf (f,
           ...
           tr->meta.color.redF (), tr->meta.color.greenF (), tr->meta.color.blueF (), MmToPts (sw),
