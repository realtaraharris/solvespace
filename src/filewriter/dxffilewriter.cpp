    ...
    if (writer->constraint) {
      for (Constraint &c : *writer->constraint) {
        ...
        case Constraint::Type::PT_PT_DISTANCE: {
          ...
          writeAlignedDimension (xfrm (ap), xfrm (bp), xfrm (ref), xfrm (ref), c.Label (),
                                 c.GetStyle (), c.valA);
          break;
        }
        ...
        case Constraint::Type::DIAMETER: {
          ...
          if (/*isRadius*/ c.other) {
            ...
            writeDiametricDimension (xfrm (center.Minus (rad)), xfrm (center.Plus (rad)),
        ...
        case Constraint::Type::ANGLE: {
          ...
          // Calculate median
          Vector m = da.WithMagnitude (1.0)
                         .ScaledBy (cos (thetaf / 2))
                         .Plus (dna.ScaledBy (sin (thetaf / 2)));
          ...
          writeAngularDimension (xfrm (a0), xfrm (a0.Plus (da)), xfrm (b0), xfrm (b0.Plus (db)),
                                 xfrm (ref), xfrm (ref), c.Label (), c.GetStyle (), c.valA);
          break;
        }
        ...
        case Constraint::Type::COMMENT: {
          ...
          writeText (xfrm (c.disp.offset), c.Label (),
                     Style::TextHeight (c.GetStyle ()) / SS.GW.scale, st->textAngle, st->textOrigin,
                     c.GetStyle ());
          break;
        }
...
void DxfFileWriter::StartPath (RgbaColor strokeRgb, double lineWidth, bool filled,
