...
void SolveSpaceUI::ExportSectionTo (const Platform::Path &filename) {
  ...
  for (auto &ent : SK.entity) {
    ...
    if (!e->IsVisible ())
      continue;
    if (e->style.v < Style::FIRST_CUSTOM)
      continue;
    if (!Style::Exportable (e->style.v))
      continue;
    if (!e->IsInPlane (n, d))
      continue;
    if (export_as_pwl) {
  ...
  if (out) {
    ...
    ExportLinesAndMesh (&el, &bl, NULL, u, v, n, origin, 0, out);
  }
  el.Clear ();
  bl.Clear ();
}
...
class GetEdgesCanvas : public Canvas {
  ...
  void DrawVectorText (const std::string &text, double height, const Vector &o, const Vector &u,
                       const Vector &v, hStroke hcs) override {
  ...
  void DrawPoint (const Vector &o, hStroke hcs) override { ssassert (false, "Not implemented"); }
  void DrawPolygon (const SPolygon &p, hFill hcf) override { ssassert (false, "Not implemented"); }
...
void SolveSpaceUI::ExportViewOrWireframeTo (const Platform::Path &filename, bool exportWireframe) {
  ...
  if (exportWireframe) {
    Vector u = Vector::From (1.0, 0.0, 0.0), v = Vector::From (0.0, 1.0, 0.0),
           n = Vector::From (0.0, 0.0, 1.0), origin = Vector::From (0.0, 0.0, 0.0);
    double cameraTan = 0.0, scale = 1.0;

    out->SetModelviewProjection (u, v, n, origin, cameraTan, scale);

    ExportWireframeCurves (&edges, &beziers, out);
  } else {
    Vector u = SS.GW.projRight, v = SS.GW.projUp, n = u.Cross (v),
           origin = SS.GW.offset.ScaledBy (-1);

    out->SetModelviewProjection (u, v, n, origin, SS.CameraTangent () * SS.GW.scale,
                                 SS.exportScale);

    ExportLinesAndMesh (&edges, &beziers, sm, u, v, n, origin, SS.CameraTangent () * SS.GW.scale,
                        out);
...
void SolveSpaceUI::ExportLinesAndMesh (SEdgeList *sel, SBezierList *sbl, SMesh *sm, Vector u,
                                       Vector v, Vector n, Vector origin, double cameraTan,
  ...
  for (int i = 0; i < sel->l.n; ++i) {
    ...
    for (int j = i + 1; j < sel->l.n; ++j) {
      ...
      if (aj.DistanceToLine (ai, di) > eps)
        continue;
      if (bj.DistanceToLine (ai, di) > eps)
        continue;
  ...
  SEdge    notClosedAt;
  sbl->l.ClearTags ();
  sblss.FindOuterFacesFrom (sbl, &spxyz, &srf, SS.ExportChordTolMm (), &allClosed, &notClosedAt,
                            NULL, NULL, &leftovers);
...
void SolveSpaceUI::ExportMeshTo (const Platform::Path &filename) {
  ...
  if (filename.HasExtension ("stl")) {
    ...
    fclose (fMtl);
  } else if (filename.HasExtension ("js") || filename.HasExtension ("html")) {
    ...
    Error ("Can't identify output file type from file extension of "
           "filename '%s'; try .stl, .obj, .js, .html.",
           filename.raw.c_str ());
  }
...
void SolveSpaceUI::ExportMeshAsObjTo (FILE *fObj, FILE *fMtl, SMesh *sm) {
  ...
  for (const STriangle &t : sm->l) {
    ...
    if (colors.find (color) == colors.end ()) {
      std::string id = ssprintf ("h%02x%02x%02x", color.red, color.green, color.blue);
...
//-----------------------------------------------------------------------------
void SolveSpaceUI::ExportMeshAsThreeJsTo (FILE *f, const Platform::Path &filename, SMesh *sm,
                                          SOutlineList *sol) {
  SPointList spl = {};
  STriangle *tr;
  Vector     bndl, bndh;
...
<html lang="en">
  ...
  if (filename.HasExtension ("html")) {
    fprintf (f, htmlbegin, THREE_FN.c_str (),
             LoadStringFromGzip ("threejs/" + THREE_FN + ".gz").c_str (), HAMMER_FN.c_str (),
             LoadStringFromGzip ("threejs/" + HAMMER_FN + ".gz").c_str (), CONTROLS_FN.c_str (),
             LoadString ("threejs/" + CONTROLS_FN).c_str ());
  }

  fprintf (f,
           "var solvespace_model_%s = {\n"
           "  bounds: {\n"
           "    x: %f, y: %f, near: %f, far: %f, z: %f, edgeBias: %f\n"
           "  },\n",
           basename.c_str (), largerBoundXY, largerBoundXY, 1.0, largerBoundZ * 2, largerBoundZ,
           largerBoundZ / 250);
  ...
  for (lightCount = 0; lightCount < 2; lightCount++) {
    fprintf (f,
             ...
             "      },\n",
             SS.lightIntensity[lightCount], CO (SS.lightDir[lightCount]));
  }
  ...
  for (sp = spl.l.First (); sp; sp = spl.l.NextAfter (sp)) {
    fprintf (f, "    [%f, %f, %f],\n", sp->p.x / SS.exportScale, sp->p.y / SS.exportScale,
  ...
  for (tr = sm->l.First (); tr; tr = sm->l.NextAfter (tr)) {
    fprintf (f, "    [%d, %d, %d],\n", spl.IndexForPoint (tr->a), spl.IndexForPoint (tr->b),
  ...
  for (const SOutline &so : sol->l) {
    ...
    fprintf (f, "    [[%f, %f, %f], [%f, %f, %f]],\n", so.a.x / SS.exportScale,
             so.a.y / SS.exportScale, so.a.z / SS.exportScale, so.b.x / SS.exportScale,
             so.b.y / SS.exportScale, so.b.z / SS.exportScale);
  }

  fputs ("  ]\n};\n", f);

  if (filename.HasExtension ("html")) {
    fprintf (f, htmlend, basename.c_str (), SS.GW.scale, CO (SS.GW.offset), CO (SS.GW.projUp),
             CO (SS.GW.projRight));
  }
...
void SolveSpaceUI::ExportMeshAsVrmlTo (FILE *f, const Platform::Path &filename, SMesh *sm) {
  ...
  for (auto &&op : opacities) {
    fprintf (f,
             ...
             basename.c_str (), (unsigned)op.first, SS.ambientIntensity, SS.ambientIntensity,
             SS.ambientIntensity, SS.ambientIntensity, 1.f - ((float)op.first / 255.0f));
    ...
    for (const auto &sp : op.second) {
      for (const auto &tr : sp) {
        fprintf (f, "          %d, %d, %d, -1,\n", spl.IndexForPoint (tr.a),
                 spl.IndexForPoint (tr.b), spl.IndexForPoint (tr.c));
      }
    }
    ...
    for (const auto &sp : op.second) {
      for (const auto &tr : sp) {
        const auto colour_itr =
            std::find_if (colours_present.begin (), colours_present.end (),
                          [&] (const RgbaColor &c) { return c.Equals (tr.meta.color); });
        if (colour_itr == colours_present.end ()) {
          fprintf (f, "          %.10f %.10f %.10f,\n", tr.meta.color.redF (),
                   tr.meta.color.greenF (), tr.meta.color.blueF ());
