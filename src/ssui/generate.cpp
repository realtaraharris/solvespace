...
bool SolveSpaceUI::PruneGroups(hGroup hg) {
  ...
  if (GroupsInOrder(g->opA, hg) && EntityExists(g->predef.origin) &&
      EntityExists(g->predef.entityB) && EntityExists(g->predef.entityC)) {
    return false;
  }
  (deleted.groups)++;
...
bool SolveSpaceUI::PruneConstraints(hGroup hg) {
  auto c = std::find_if(SK.constraint.begin(), SK.constraint.end(), [&](Constraint &c) {
    ...
    if (EntityExists(c.workplane) && EntityExists(c.ptA) && EntityExists(c.ptB) &&
        EntityExists(c.entityA) && EntityExists(c.entityB) && EntityExists(c.entityC) &&
...
void SolveSpaceUI::GenerateAll(Generate type, bool andFindFree, bool genForBBox) {
  ...
  uint64_t startMillis = GetMilliseconds(), endMillis;

  SK.groupOrder.Clear();
  for (auto &g : SK.group) {
    SK.groupOrder.Add(&g.h);
  }
  std::sort(SK.groupOrder.begin(), SK.groupOrder.end(), [](const hGroup &ha, const hGroup &hb) {
...
*/
    ...
    // constraints.
    if (deleted.requests > 0 || deleted.nonTrivialConstraints > 0 || deleted.groups > 0) {
      ...
      Message("Additional sketch elements were deleted, because they "
              ...
              "     %d group%s"
              "%s",
              deleted.requests, deleted.requests == 1 ? "" : "s", deleted.constraints,
              deleted.constraints == 1 ? "" : "s", deleted.groups, deleted.groups == 1 ? "" : "s",
  ...
  if (endMillis - startMillis > 30) {
    ...
    if (endMillis)
      dbp("Generate::%s%s took %lld ms", typeStr, (genForBBox ? " (for bounding box)" : ""),
...
void SolveSpaceUI::MarkDraggedParams() {
  ...
  if (SS.GW.pending.circle.v) {
    ...
    if (circ) {
      ...
      switch (dist->type) {
      case Entity::Type::DISTANCE: sys.dragged.Add(&(dist->param[0])); break;
