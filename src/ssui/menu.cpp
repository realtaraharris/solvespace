...
void SolveSpaceUI::MenuView (Command id) {
  switch (id) {
  case Command::ZOOM_IN: SS.GW.ZoomToMouse (1); break;

  case Command::ZOOM_OUT: SS.GW.ZoomToMouse (-1); break;
      ...
      for (j = 0; j < 3; j++) {
        ...
        for (negi = 0; negi < 2; negi++) {
          for (negj = 0; negj < 2; negj++) {
            ...
            // TODO: do this only if we're animating. otherwise these lines flip the camera
            // incorrectly if(negi) ou = ou.ScaledBy(-1); if(negj) ov = ov.ScaledBy(-1);
            ...
            if (id == Command::NEAREST_ORTHO || id == Command::ONTO_WORKPLANE) {
              ...
              u = ou.ScaledBy (1 / sqrt2).Plus (on.ScaledBy (-1 / sqrt2));
              v = ou.ScaledBy (-1 / sqrt6)
                      .Plus (ov.ScaledBy (2 / sqrt6).Plus (on.ScaledBy (-1 / sqrt6)));
            }

            Quaternion quatt = Quaternion::From (u, v);
            double     d =
                std::min ((quatt.Minus (quat0)).Magnitude (), (quatt.Plus (quat0)).Magnitude ());
            if (d < dmin) {
              dmin  = d;
              quatf = quatt;
            }
...
void SolveSpaceUI::MenuEdit (Command id) {
    ...
    do {
      ...
      for (Entity &e : SK.entity) {
        ...
        for (GraphicsWindow::Selection *s = ls->First (); s; s = ls->NextAfter (s)) {
          ...
          if (sst.Equals (st) || sst.Equals (fi) || sfi.Equals (st) || sfi.Equals (fi)) {
            onChain = true;
          }
        }
  ...
  case Command::UNDO: SS.UndoUndo (); break;

  case Command::REDO: SS.UndoRedo (); break;
...
void SolveSpaceUI::MenuClipboard (Command id) {
  ...
  case Command::PASTE: {
    SS.UndoRemember ();
    Vector trans =
        SS.GW.projRight.ScaledBy (80 / SS.GW.scale).Plus (SS.GW.projUp.ScaledBy (40 / SS.GW.scale));
    SS.GW.ClearSelection ();
...
void SolveSpaceUI::MenuRequest (Command id) {
  ...
  case Command::CONSTR_SEGMENT: s = _ ("click first point of construction line segment"); goto c;
...
void SolveSpaceUI::MenuConstrain (Command id) {
  ...
  case Command::REF_ANGLE: {
    ...
    if (ea->type == Entity::Type::LINE_SEGMENT && eb->type == Entity::Type::LINE_SEGMENT) {
  ...
  case Command::PARALLEL:
    if (gs.faces == 2 && gs.n == 2) {
      ...
      Entity *line = SK.GetEntity (gs.entity[0]), *arc = SK.GetEntity (gs.entity[1]);
      ...
      Entity *line = SK.GetEntity (gs.entity[0]), *cubic = SK.GetEntity (gs.entity[1]);
      ...
      Entity *eA = SK.GetEntity (gs.entity[0]), *eB = SK.GetEntity (gs.entity[1]);
  ...
  if (SK.constraint.FindByIdNoOops (c.h)) {
    ...
    if (SS.TestRankForGroup (c.group) == SolveResult::REDUNDANT_OKAY &&
        !SK.GetGroup (SS.GW.activeGroup)->allowRedundant && constraint->HasLabel ()) {
      constraint->reference = true;
    }
  }

  if ((id == Command::DISTANCE_DIA || id == Command::ANGLE || id == Command::RATIO ||
       id == Command::DIFFERENCE) &&
