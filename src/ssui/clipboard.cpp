...
void GraphicsWindow::CopySelection() {
  ...
  Vector u = wrkpln->NormalU(), v = wrkpln->NormalV(), n = wrkpln->NormalN(),
  ...
  for (Selection *s = ls->First(); s; s = ls->NextAfter(s)) {
    ...
    Request::Type req;
    int pts;
    if (!EntReqTable::GetEntityInfo(e->type, e->extraPoints, &req, &pts, NULL, &hasDistance)) {
      if (!e->h.isFromRequest())
        continue;
      Request *r = SK.GetRequest(e->h.request());
      if (r->type != Request::Type::DATUM_POINT)
        continue;
      EntReqTable::GetEntityInfo((Entity::Type)0, e->extraPoints, &req, &pts, NULL, &hasDistance);
    }
    if (req == Request::Type::WORKPLANE)
      continue;
...
void GraphicsWindow::PasteClipboard(Vector trans, double theta, double scale) {
  ...
  for (cr = SS.clipboard.r.First(); cr; cr = SS.clipboard.r.NextAfter(cr)) {
    ...
    bool hasDistance;
    int i, pts;
    EntReqTable::GetRequestInfo(r->type, r->extraPoints, NULL, &pts, NULL, &hasDistance);
    for (i = 0; i < pts; i++) {
    ...
    if (hasDistance) {
      SK.GetEntity(hr.entity(64))->DistanceForceTo(cr->distance * fabs(scale));
    }
  ...
  for (cc = SS.clipboard.c.First(); cc; cc = SS.clipboard.c.NextAfter(cc)) {
    ...
    switch (c.type) {
    case Constraint::Type::COMMENT: c.disp.offset = c.disp.offset.Plus(trans); break;
    case Constraint::Type::PT_PT_DISTANCE:
    case Constraint::Type::PT_LINE_DISTANCE:
    case Constraint::Type::PROJ_PT_DISTANCE:
    case Constraint::Type::DIAMETER: c.valA *= fabs(scale); break;
    case Constraint::Type::ARC_LINE_TANGENT: {
      Entity *line = SK.GetEntity(c.entityB), *arc = SK.GetEntity(c.entityA);
    ...
    case Constraint::Type::CUBIC_LINE_TANGENT: {
      Entity *line = SK.GetEntity(c.entityB), *cubic = SK.GetEntity(c.entityA);
...
bool TextWindow::EditControlDoneForPaste(const std::string &s) {
  ...
  case Edit::PASTE_TIMES_REPEATED: {
    e = Expr::From(s, true); // popUpError = true
    if (!e)
      break;
    int v = (int)e->Eval();
    if (v > 0) {
...
void TextWindow::ScreenChangePasteTransformed(int link, uint32_t v) {
  ...
  case 'f': SS.TW.shown.paste.scale *= -1; break;
  }
}
...
void TextWindow::ScreenPasteTransformed(int link, uint32_t v) {
  ...
  case 't':
    if (SS.GW.gs.points == 2 && SS.GW.gs.n == 2) {
      Entity *pa = SK.GetEntity(SS.GW.gs.point[0]), *pb = SK.GetEntity(SS.GW.gs.point[1]);
      SS.TW.shown.paste.trans = (pb->PointGetNum()).Minus(pa->PointGetNum());
    } else {
  ...
  case 'g': {
    ...
    for (int i = 0; i < SS.TW.shown.paste.times; i++) {
      Vector trans = SS.TW.shown.paste.trans.ScaledBy(i + 1), origin = SS.TW.shown.paste.origin;
      double theta = SS.TW.shown.paste.theta * (i + 1);
      // desired transformation is Q*(p - o) + o + t =
      // Q*p - Q*o + o + t = Q*p + (t + o - Q*o)
      Vector t = trans.Plus(origin).Minus(origin.RotatedAbout(wn, theta));
...
void TextWindow::ShowPasteTransformed() {
  ...
  Printf(true, "%Ba   %Ftrepeat%E    %d time%s %Fl%Lt%f[change]%E", shown.paste.times,
         (shown.paste.times == 1) ? "" : "s", &ScreenChangePasteTransformed);
  Printf(false, "%Bd   %Ftrotate%E    %@ degrees %Fl%Lr%f[change]%E", shown.paste.theta * 180 / PI,
         &ScreenChangePasteTransformed);
  Printf(false, "%Ba   %Ftabout pt%E  (%s, %s, %s) %Fl%Lo%f[use selected]%E",
         SS.MmToString(shown.paste.origin.x).c_str(), SS.MmToString(shown.paste.origin.y).c_str(),
         SS.MmToString(shown.paste.origin.z).c_str(), &ScreenPasteTransformed);
  Printf(false, "%Bd   %Fttranslate%E (%s, %s, %s) %Fl%Lt%f[use selected]%E",
         SS.MmToString(shown.paste.trans.x).c_str(), SS.MmToString(shown.paste.trans.y).c_str(),
         SS.MmToString(shown.paste.trans.z).c_str(), &ScreenPasteTransformed);
  Printf(false, "%Ba   %Ftscale%E     %@ %Fl%Ls%f[change]%E", fabs(shown.paste.scale),
         &ScreenChangePasteTransformed);
  Printf(false, "%Ba   %Ftmirror%E    %Fd%Lf%f%s  flip%E", &ScreenChangePasteTransformed,
