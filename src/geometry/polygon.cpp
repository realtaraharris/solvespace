...
bool SEdge::EdgeCrosses(Vector ea, Vector eb, Vector *ppi, SPointList *spl) const {
  ...
  if (inOrEdge0 && inOrEdge1) {
    if (a.Equals(ea) || b.Equals(ea) || a.Equals(eb) || b.Equals(eb)) {
...
bool SEdgeList::AssembleContour(Vector first, Vector last, SContour *dest, SEdge *errorAt,
                                bool keepDir, int start) const {
  int i;
...
int SKdNodeEdges::AnyEdgeCrossings(Vector a, Vector b, int cnt, Vector *pi, SPointList *spl) const {
  int inters = 0;
  if (gt && lt) {
    if (a.Element(which) < c + KDTREE_EPS || b.Element(which) < c + KDTREE_EPS) {
...
//-----------------------------------------------------------------------------
static bool IntersectionOfLines(double x0A, double y0A, double dxA, double dyA, double x0B,
                                double y0B, double dxB, double dyB, double *xi, double *yi) {
  double A[2][2];
  double b[2];
...
void SContour::OffsetInto(SContour *dest, double r) const {
  ...
  for (i = 0; i < l.n; i++) {
    ...
    if (std::fabs(thetan - thetap) < (1 * PI) / 180) {
      ...
      IntersectionOfLines(px0, py0, pdx, pdy, nx0, ny0, ndx, ndy, &x, &y);

      dest->AddPoint(Vector::From(x, y, 0));
    } else {
      if (std::fabs(thetap - thetan) < (6 * PI) / 180) {
        Vector pp = Vector(b.x - r * sin(thetap), b.y + r * cos(thetap), 0);
        dest->AddPoint(pp);

        Vector pn = Vector(b.x - r * sin(thetan), b.y + r * cos(thetan), 0);
        dest->AddPoint(pn);
      } else {
        double theta;
        for (theta = thetap; theta <= thetan; theta += (6 * PI) / 180) {
          Vector p = Vector(b.x - r * sin(theta), b.y + r * cos(theta), 0);
          dest->AddPoint(p);
        }
      }
    }
