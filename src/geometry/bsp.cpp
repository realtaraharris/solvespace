...
SBsp2 *SBsp2::Alloc() {
  return (SBsp2 *)AllocTemporary(sizeof(SBsp2));
}
SBsp3 *SBsp3::Alloc() {
  return (SBsp3 *)AllocTemporary(sizeof(SBsp3));
}

SBsp3 *SBsp3::FromMesh(const SMesh *m) {
  SMesh mc = {};
  for (auto const &elt : m->l) {
    mc.AddTriangle(&elt);
  }
  ...
  SBsp3 *bsp3 = NULL;
  for (auto &elt : mc.l) {
    bsp3 = InsertOrCreate(bsp3, &elt, NULL);
  }
  mc.Clear();
  return bsp3;
}
...
void SBsp3::InsertInPlane(bool pos2, STriangle *tr, SMesh *m) {
  ...
  if ((!onFace && ((m->keepInsideOtherShell && !pos2) || (!m->keepInsideOtherShell && pos2))) ||
...
class BspUtil {
  ...
  static BspUtil *Alloc() { return (BspUtil *)AllocTemporary(sizeof(BspUtil)); }

  void AllocOn() { on = (Vector *)AllocTemporary(sizeof(Vector) * 2); }

  void AllocTriangle() { btri = (STriangle *)AllocTemporary(sizeof(STriangle)); }
  ...
  void AllocQuad() { vpos = (Vector *)AllocTemporary(sizeof(Vector) * 4); }
  ...
  void ClassifyTriangle(STriangle *tri, SBsp3 *node) {
    ...
    for (int i = 0; i < 3; i++) {
      if (dt[i] > d + SolveSpace::LENGTH_EPS) {
        posc++;
        isPos[i] = true;
      } else if (dt[i] < d - SolveSpace::LENGTH_EPS) {
        negc++;
        isNeg[i] = true;
      } else {
        onc++;
  ...
  static SBsp3 *Triangulate(SBsp3 *bsp, const STriMeta &meta, Vector *vertex, size_t cnt,
                            SMesh *instead) {
...
void SBsp3::InsertConvexHow(BspClass how, STriMeta meta, Vector *vertex, size_t n, SMesh *instead) {
  switch (how) {
  case BspClass::POS:
    if (pos) {
