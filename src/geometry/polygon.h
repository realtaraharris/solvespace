...
enum class EarType : uint32_t { UNKNOWN = 0, NOT_EAR = 1, EAR = 2 };

enum class BspClass : uint32_t { POS = 100, NEG = 101, COPLANAR = 200 };
...
class SEdgeList {
  ...
  bool AssembleContour (Vector first, Vector last, SContour *dest, SEdge *errorAt, bool keepDir,
                        int start) const;
  int  AnyEdgeCrossings (Vector a, Vector b, Vector *pi = NULL, SPointList *spl = NULL) const;
...
class SKdNodeEdges {
  ...
  int                  AnyEdgeCrossings (Vector a, Vector b, int cnt, Vector *pi = NULL,
...
class SBsp3 {
  ...
  void   InsertConvexHow (BspClass how, STriMeta meta, Vector *vertex, size_t n, SMesh *instead);
...
class SMesh {
  ...
  void MakeFromTransformationOf (SMesh *a, Vector trans, Quaternion q, double scale);
...
class SKdNode {
  ...
  void MakeCertainEdgesInto (SEdgeList *sel, EdgeKind how, bool coplanarIsInter, bool *inter,
                             bool *leaky, int auxA = 0) const;
