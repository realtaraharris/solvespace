...
void SMesh::Simplify(int start) {
  ...
  for (;;) {
    ...
    do {
      ...
      for (j = 0; j < convc; j++) {
        Vector a = conv[WRAP((j - 1), convc)], b = conv[j], d = conv[WRAP((j + 1), convc)],
...
void SMesh::MakeFromTransformationOf(SMesh *a, Vector trans, Quaternion q, double scale) {
  STriangle *tr;
...
SKdNode *SKdNode::From(STriangleLl *tll) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (ll = tll; ll; ll = ll->next) {
      ...
      double a = (tr->a).Element(i), b = (tr->b).Element(i), c = (tr->c).Element(i);

      if (a < split[i] + KDTREE_EPS || b < split[i] + KDTREE_EPS || c < split[i] + KDTREE_EPS) {
        ltc[i]++;
      }
      if (a > split[i] - KDTREE_EPS || b > split[i] - KDTREE_EPS || c > split[i] - KDTREE_EPS) {
        gtc[i]++;
  ...
  for (ll = tll; ll; ll = ll->next) {
    ...
    double a = (tr->a).Element(which), b = (tr->b).Element(which), c = (tr->c).Element(which);
...
void SKdNode::AddTriangle(STriangle *tr) {
  if (gt && lt) {
    double ta = (tr->a).Element(which), tb = (tr->b).Element(which), tc = (tr->c).Element(which);
    if (ta < c + KDTREE_EPS || tb < c + KDTREE_EPS || tc < c + KDTREE_EPS) {
      lt->AddTriangle(tr);
    }
    if (ta > c - KDTREE_EPS || tb > c - KDTREE_EPS || tc > c - KDTREE_EPS) {
      gt->AddTriangle(tr);
...
void SKdNode::ListTrianglesInto(std::vector<STriangle *> *tl) const {
  if (gt)
    gt->ListTrianglesInto(tl);
  if (lt)
    lt->ListTrianglesInto(tl);
...
void SKdNode::SplitLinesAgainstTriangle(SEdgeList *sel, STriangle *tr) const {
  ...
  if (tn.z > LENGTH_EPS) {
    ...
    for (se = sel->l.First(); se; se = sel->l.NextAfter(se)) {
      double da = (se->a).Dot(tn) - td, db = (se->b).Dot(tn) - td;
      if ((da < -LENGTH_EPS && db > LENGTH_EPS) || (db < -LENGTH_EPS && da > LENGTH_EPS)) {
        Vector m = VectorAtIntersectionOfPlaneAndLine(tn, td, se->a, se->b, NULL);
    ...
    // triangle.
    Point2d a = (tr->a).ProjectXy(), b = (tr->b).ProjectXy(), c = (tr->c).ProjectXy();

    Point2d n[3] = {(b.Minus(a)).Normal().WithMagnitude(1), (c.Minus(b)).Normal().WithMagnitude(1),
                    (a.Minus(c)).Normal().WithMagnitude(1)};

    double d[3] = {n[0].Dot(b), n[1].Dot(c), n[2].Dot(a)};
...
void SKdNode::FindEdgeOn(Vector a, Vector b, int cnt, bool coplanarIsInter,
  ...
  if (gt && lt) {
    double ac = a.Element(which), bc = b.Element(which);
    if (ac < c + KDTREE_EPS || bc < c + KDTREE_EPS) {
  ...
  for (ll = tris; ll; ll = ll->next) {
    ...
    if ((a.Equals(tr->b) && b.Equals(tr->a)) || (a.Equals(tr->c) && b.Equals(tr->b)) ||
    ...
    } else if (((a.Equals(tr->a) && b.Equals(tr->b)) || (a.Equals(tr->b) && b.Equals(tr->c)) ||
      ...
      if ((pa < -LENGTH_EPS || pa > LENGTH_EPS) && (pb < -LENGTH_EPS || pb > LENGTH_EPS) &&
          (pa * pb < 0)) {
...
//-----------------------------------------------------------------------------
void SKdNode::MakeCertainEdgesInto(SEdgeList *sel, EdgeKind how, bool coplanarIsInter, bool *inter,
                                   bool *leaky, int auxA) const {
  if (inter)
    *inter = false;
  if (leaky)
    *leaky = false;
  ...
  for (STriangle *tr : tris) {
    for (int j = 0; j < 3; j++) {
      ...
      case EdgeKind::TURNING:
        if ((tr->Normal().z < LENGTH_EPS) && (info.count == 1) && info.frontFacing) {
      ...
      case EdgeKind::SHARP:
        if (info.count == 1) {
          ...
          if (!((na0.Equals(na1) && nb0.Equals(nb1)) || (na0.Equals(nb1) && nb0.Equals(na1)))) {
            if (CheckAndAddTrianglePair(&edgeTris, tr, info.tr))
              break;
...
void SKdNode::MakeOutlinesInto(SOutlineList *sol, EdgeKind edgeKind) const {
  ...
  for (STriangle *tr : tris) {
    for (int j = 0; j < 3; j++) {
      ...
      default: ssassert(false, "Unexpected edge kind");
      }
...
bool SOutline::IsVisible(Vector projDir) const {
  ...
  return (ldot > -LENGTH_EPS) == (rdot < LENGTH_EPS) || (rdot > -LENGTH_EPS) == (ldot < LENGTH_EPS);
}
...
double SMesh::CalculateVolume() const {
  ...
  for (STriangle tr : l) {
    ...
    double integral =
        (1.0 / 3) * (A * (mbc - mac) + (1.0 / 2) * B * (mbc * mbc - mac * mac)) * (xc * xc * xc) +
        (1.0 / 2) * (A * yb + B * yb * mbc + C * (mbc - mac)) * xc * xc + C * yb * xc +
        (1.0 / 2) * B * yb * yb * xc;
