...
void GraphicsWindow::Selection::Draw(bool isHovered, Canvas *canvas) {
  ...
  if (entity.v) {
    Entity *e = SK.GetEntity(entity);
    e->Draw(isHovered ? Entity::DrawAs::HOVERED : Entity::DrawAs::SELECTED, canvas);
    if (emphasized) {
  ...
  if (constraint.v) {
    ...
    c->Draw(isHovered ? Constraint::DrawAs::HOVERED : Constraint::DrawAs::SELECTED, canvas);
    if (emphasized) {
...
GraphicsWindow::Selection GraphicsWindow::ChooseFromHoverToSelect() {
  ...
  for (const Hover &hov : hoverList) {
    ...
    if (g->order > activeGroup->order)
      continue;
    if (bestOrder != -1 && (bestOrder > g->order || bestZIndex > hov.zIndex))
      continue;
...
void GraphicsWindow::HitTestMakeSelection(Point2d mp) {
  ...
  for (Entity &e : SK.entity) {
    ...
    if (e.type == Entity::Type::IMAGE && !showFaces)
      continue;
...
Vector GraphicsWindow::UnProjectPoint(Point2d p) {
  ...
  // correct.
  orig = orig.Plus(projRight.ScaledBy(p.x / scale)).Plus(projUp.ScaledBy(p.y / scale));
  return orig;
}
...
void GraphicsWindow::DrawSnapGrid(Canvas *canvas) {
  ...
  for (i = i0 + 1; i < i1; i++) {
    canvas->DrawLine(wp.Plus(wu.ScaledBy(i * g)).Plus(wv.ScaledBy(j0 * g)),
                     wp.Plus(wu.ScaledBy(i * g)).Plus(wv.ScaledBy(j1 * g)), hcs);
  }
  for (j = j0 + 1; j < j1; j++) {
    canvas->DrawLine(wp.Plus(wu.ScaledBy(i0 * g)).Plus(wv.ScaledBy(j * g)),
                     wp.Plus(wu.ScaledBy(i1 * g)).Plus(wv.ScaledBy(j * g)), hcs);
  }
}

void GraphicsWindow::DrawEntities(Canvas *canvas, bool persistent) {
  for (Entity &e : SK.entity) {
    if (persistent == (e.IsNormal() || e.IsWorkplane()))
      continue;
    switch (SS.GW.drawOccludedAs) {
    case DrawOccludedAs::VISIBLE: e.Draw(Entity::DrawAs::OVERLAY, canvas); break;

    case DrawOccludedAs::STIPPLED:
      e.Draw(Entity::DrawAs::HIDDEN, canvas);
      /* fallthrough */
    case DrawOccludedAs::INVISIBLE: e.Draw(Entity::DrawAs::DEFAULT, canvas); break;
    }
  }
}
...
void GraphicsWindow::Draw(Canvas *canvas) {
  ...
  if (SS.justExportedInfo.draw) {
    ...
    if (SS.justExportedInfo.showOrigin) {
      p = SS.justExportedInfo.pt, u = SS.justExportedInfo.u, v = SS.justExportedInfo.v;
    } else {
    ...
    if (SS.justExportedInfo.showOrigin) {
      Vector um = p.Plus(u.WithMagnitude(-15 / scale)), up = p.Plus(u.WithMagnitude(30 / scale)),
             vm = p.Plus(v.WithMagnitude(-15 / scale)), vp = p.Plus(v.WithMagnitude(30 / scale));
      canvas->DrawLine(um, up, hcsDatum);
      canvas->DrawLine(vm, vp, hcsDatum);
      canvas->DrawVectorText(
          "(x, y) = (0, 0) for file just exported", Style::DefaultTextHeight() / camera.scale,
          p.Plus(u.ScaledBy(40 / scale)).Plus(v.ScaledBy(-(Style::DefaultTextHeight()) / scale)), u,
          v, hcsDatum);
    }
  }
}
...
void GraphicsWindow::Paint() {
  ...
  if (pending.operation == Pending::DRAGGING_MARQUEE) {
    ...
    uiCanvas.DrawRect(
        (int)orig.mouse.x + (int)camera.width / 2, (int)begin.x + (int)camera.width / 2,
        (int)orig.mouse.y + (int)camera.height / 2, (int)begin.y + (int)camera.height / 2,
