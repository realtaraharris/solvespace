...
Expr *ExprQuaternion::Magnitude() const {
  return ((w->Square())->Plus((vx->Square())->Plus((vy->Square())->Plus((vz->Square())))))->Sqrt();
}
...
Expr *Expr::PartialWrt(hParam p) const {
  ...
  case Op::SQRT: return (From(0.5)->Div(a->Sqrt()))->Times(a->PartialWrt(p));

  case Op::SQUARE: return (From(2.0)->Times(a))->Times(a->PartialWrt(p));
  ...
  case Op::ASIN:
    return (From(1)->Div((From(1)->Minus(a->Square()))->Sqrt()))->Times(a->PartialWrt(p));
  case Op::ACOS:
    return (From(-1)->Div((From(1)->Minus(a->Square()))->Sqrt()))->Times(a->PartialWrt(p));
  }
  ssassert(false, "Unexpected operation");
...
void Expr::ParamsUsedList(std::vector<hParam> *list) const {
  if (op == Op::PARAM || op == Op::PARAM_PTR) {
    ...
    if (list->end() !=
        std::find_if(list->begin(), list->end(), [=](const hParam &p) { return p.v == param.v; })) {
...
ExprParser::Token ExprParser::PopOperator(std::string *error) {
  Token t = Token::From();
  if (stack.empty() ||
      (stack.back().type != TokenType::UNARY_OP && stack.back().type != TokenType::BINARY_OP)) {
...
bool ExprParser::Parse(std::string *error, size_t reduceUntil) {
  while (true) {
    Token t = Lex(error);
    switch (t.type) {
    case TokenType::ERROR: return false;
    ...
    case TokenType::UNARY_OP:
    case TokenType::OPERAND: stack.push_back(t); break;
    }
  }
