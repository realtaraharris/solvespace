...
void TextWindow::ShowHeader(bool withNav) {
  ...
  // Navigation buttons
  if (withNav) {
    Printf(false, " %Fl%Lh%fhome%E   %Ft%s%E%s", (&TextWindow::ScreenHome), header, desc.c_str());
  } else {
...
void TextWindow::ShowListOfGroups() {
  const char *radioTrue = " " RADIO_TRUE " ", *radioFalse = " " RADIO_FALSE " ",
             *checkTrue = " " CHECK_TRUE " ", *checkFalse = " " CHECK_FALSE " ";
  ...
  for (hGroup hg : SK.groupOrder) {
    ...
    Printf(false,
           ...
           ref ? "   " : "", g->h.v, (&TextWindow::ScreenActivateGroup),
           ref ? "" : (active ? radioTrue : radioFalse),
           // Link that hides or shows the group
           afterActive ? " - " : "", g->h.v, (&TextWindow::ScreenToggleGroupShown),
           afterActive ? "" : (shown ? checkTrue : checkFalse),
           // Link to the errors, if a problem occurred while solving
           ok ? (warn ? 'm' : (dof > 0 ? 'i' : 's')) : 'x', g->h.v,
           (&TextWindow::ScreenHowGroupSolved),
           ok ? ((warn && SS.checkClosedContour) ? "err" : sdof) : "", ok ? "" : "ERR",
           // Link to a screen that gives more details on the group
           g->suppress ? 'g' : 'l', g->h.v, (&TextWindow::ScreenSelectGroup), s.c_str(),
           suffix.c_str());
  ...
  Printf(true, "  %Fl%Ls%fshow all%E / %Fl%Lh%fhide all%E", &(TextWindow::ScreenShowGroupsSpecial),
         &(TextWindow::ScreenShowGroupsSpecial));
  Printf(true,
         "  %Fl%Ls%fline styles%E /"
         " %Fl%Ls%fview%E /"
         " %Fl%Ls%fconfiguration%E",
         &(TextWindow::ScreenShowListOfStyles), &(TextWindow::ScreenShowEditView),
...
void TextWindow::ScreenChangePitchOption(int link, uint32_t v) {
  Group *g = SK.GetGroup(SS.TW.shown.group);
  if (g->valB == 0.0) {
    g->valB = SK.GetParam(g->h.param(7))->val * PI / (SK.GetParam(g->h.param(3))->val);
  } else {
    g->valB = 0.0;
  }
  SS.GW.Invalidate();
...
void TextWindow::ShowGroupInfo() {
  ...
  if (shown.group == Group::HGROUP_REFERENCES) {
    ...
    Printf(true, "%FtGROUP  %E%s [%Fl%Ll%D%frename%E/%Fl%Ll%D%fdel%E]",
           g->DescriptionString().c_str(), g->h.v, &TextWindow::ScreenChangeGroupName, g->h.v,
           &TextWindow::ScreenDeleteGroup);
  }
  ...
  if (g->type == Group::Type::LATHE) {
    ...
    Printf(false,
           ...
           &TextWindow::ScreenChangeGroupOption, one ? RADIO_TRUE : RADIO_FALSE,
           &TextWindow::ScreenChangeGroupOption, !one ? RADIO_TRUE : RADIO_FALSE);
    ...
    if (g->type == Group::Type::ROTATE || g->type == Group::Type::TRANSLATE) {
      if (g->subtype == Group::Subtype::ONE_SIDED) {
        ...
        Printf(false,
               ...
               &ScreenChangeGroupOption, !skip ? RADIO_TRUE : RADIO_FALSE, &ScreenChangeGroupOption,
               skip ? RADIO_TRUE : RADIO_FALSE);
      }

      int times = (int)(g->valA);
      Printf(false, "%Bp   %Ftrepeat%E %d time%s %Fl%Ll%D%f[change]%E",
             (g->subtype == Group::Subtype::ONE_SIDED) ? 'a' : 'd', times, times == 1 ? "" : "s",
    ...
    Printf(false, "%Bd   %Ftscaled by%E %# %Fl%Ll%f%D[change]%E", g->scale,
  ...
  if (g->type == Group::Type::HELIX) {
    ...
    if (fabs(g->valB) != 0.0) {
      Printf(false, "  %Ba %# %Fl%Ll%f%D[change]%E", g->valB / SS.MmPerUnit(),
    ...
    Printf(false, "   %Fd%f%LP%s  fixed", &TextWindow::ScreenChangePitchOption,
  ...
  if (g->type == Group::Type::EXTRUDE || g->type == Group::Type::LATHE ||
    ...
    Printf(false,
           "%Ba   %f%D%Lc%Fd%s union%E  "
           "%f%D%Lc%Fd%s assemble%E  ",
           &TextWindow::ScreenChangeGroupOption, Group::CombineAs::UNION,
           un ? RADIO_TRUE : RADIO_FALSE, &TextWindow::ScreenChangeGroupOption,
           Group::CombineAs::ASSEMBLE, (asy ? RADIO_TRUE : RADIO_FALSE));
    Printf(false,
           "%Ba   %f%D%Lc%Fd%s difference%E  "
           "%f%D%Lc%Fd%s intersection%E  ",
           &TextWindow::ScreenChangeGroupOption, Group::CombineAs::DIFFERENCE,
           diff ? RADIO_TRUE : RADIO_FALSE, &TextWindow::ScreenChangeGroupOption,
           Group::CombineAs::INTERSECTION, intr ? RADIO_TRUE : RADIO_FALSE);

    if (g->type == Group::Type::EXTRUDE || g->type == Group::Type::LATHE ||
        g->type == Group::Type::REVOLVE || g->type == Group::Type::HELIX) {
      Printf(false, "%Bd   %Ftcolor   %E%Bz  %Bd (%@, %@, %@) %f%D%Lf%Fl[change]%E", &g->color,
             g->color.redF(), g->color.greenF(), g->color.blueF(), ScreenColor, top[rows - 1] + 2);
      Printf(false, "%Bd   %Ftopacity%E %@ %f%Lf%Fl[change]%E", g->color.alphaF(),
    ...
    if (g->type == Group::Type::EXTRUDE || g->type == Group::Type::LATHE ||
      ...
      Printf(false, "   %Fd%f%LP%s  suppress this group's solid model",
             &TextWindow::ScreenChangeGroupOption, g->suppress ? CHECK_TRUE : CHECK_FALSE);
    }

    Printf(false, "");
  }

  Printf(false, " %f%Lv%Fd%s  show entities from this group", &TextWindow::ScreenChangeGroupOption,
  ...
  if (!g->IsForcedToMeshBySource() && !g->IsTriangleMeshAssembly()) {
    Printf(false, " %f%Lf%Fd%s  force NURBS surfaces to triangle mesh",
           &TextWindow::ScreenChangeGroupOption, g->forceToMesh ? CHECK_TRUE : CHECK_FALSE);
  } else {
    Printf(false, " (model already forced to triangle mesh)");
  }

  Printf(true, " %f%Lr%Fd%s  relax constraints and dimensions",
         &TextWindow::ScreenChangeGroupOption, g->relaxConstraints ? CHECK_TRUE : CHECK_FALSE);

  Printf(false, " %f%Le%Fd%s  allow redundant constraints", &TextWindow::ScreenChangeGroupOption,
  ...
  Printf(false, " %f%Ld%Fd%s  treat all dimensions as reference",
         &TextWindow::ScreenChangeGroupOption, g->allDimsReference ? CHECK_TRUE : CHECK_FALSE);
...
list_items:
  ...
  for (auto &r : SK.request) {
    ...
    if (r.group == shown.group) {
      ...
      Printf(false, "%Bp   %Fl%Ll%D%f%h%s%E", (a & 1) ? 'd' : 'a', r.h.v,
             (&TextWindow::ScreenSelectRequest), &(TextWindow::ScreenHoverRequest), s.c_str());
      a++;
    }
  }
  if (a == 0)
  ...
  for (auto &c : SK.constraint) {
    ...
    if (c.group == shown.group) {
      ...
      Printf(false, "%Bp   %Fl%Ll%D%f%h%s%E %s", (a & 1) ? 'd' : 'a', c.h.v,
             (&TextWindow::ScreenSelectConstraint), (&TextWindow::ScreenHoverConstraint), s.c_str(),
             c.reference ? "(ref)" : "");
      a++;
    }
  }
...
void TextWindow::ShowGroupSolveInfo() {
  ...
  case SolveResult::TOO_MANY_UNKNOWNS: Printf(true, "Too many unknowns in a single group!"); return;
  ...
  for (int i = 0; i < g->solved.remove.n; i++) {
    ...
    Printf(false, "%Bp   %Fl%Ll%D%f%h%s%E", (i & 1) ? 'd' : 'a', c->h.v,
...
void TextWindow::ShowStepDimension() {
  ...
  if (stepDim.isDistance) {
    ...
    Printf(false, "%Bd   %Ftfinish%E   %@ %Fl%Ll%f[change]%E", stepDim.finish,
           &ScreenStepDimFinish);
  }
  Printf(false, "%Ba   %Ftsteps%E    %d %Fl%Ll%f%D[change]%E", stepDim.steps, &ScreenStepDimSteps);
...
void TextWindow::ShowTangentArc() {
  ...
  if (SS.tangentArcManual) {
    Printf(false, "%Ba   %s %Fl%Lr%f[change]%E", SS.MmToString(SS.tangentArcRadius).c_str(),
  ...
  Printf(false, "");
  Printf(false, "  %Fd%f%La%s  choose radius automatically%E", &ScreenChangeTangentArc,
         !SS.tangentArcManual ? CHECK_TRUE : CHECK_FALSE);
  Printf(false, "  %Fd%f%Lm%s  modify original entities%E", &ScreenChangeTangentArc,
...
void TextWindow::EditControlDone(std::string s) {
  ...
  case Edit::GROUP_COLOR: {
    ...
    if (sscanf(s.c_str(), "%lf, %lf, %lf", &rgb.x, &rgb.y, &rgb.z) == 3) {
      ...
      if (!g)
        break;
      g->color = RgbaColor::FromFloat((float)rgb.x, (float)rgb.y, (float)rgb.z, g->color.alphaF());
  ...
  case Edit::STEP_DIM_STEPS: stepDim.steps = std::min(300, std::max(1, atoi(s.c_str()))); break;
  ...
  default: {
    int cnt = 0;
    if (EditControlDoneForStyles(s))
      cnt++;
    if (EditControlDoneForConfiguration(s))
      cnt++;
    if (EditControlDoneForPaste(s))
      cnt++;
    if (EditControlDoneForView(s))
      cnt++;
