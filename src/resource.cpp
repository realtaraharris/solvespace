...
namespace SolveSpace {
  ...
  RgbaColor Pixmap::GetPixel(size_t x, size_t y) const {
    ...
    switch (format) {
    case Format::RGBA: return RgbaColor::From(pixel[0], pixel[1], pixel[2], pixel[3]);

    case Format::RGB: return RgbaColor::From(pixel[0], pixel[1], pixel[2], 255);

    case Format::BGRA: return RgbaColor::From(pixel[2], pixel[1], pixel[0], pixel[3]);

    case Format::BGR: return RgbaColor::From(pixel[2], pixel[1], pixel[0], 255);

    case Format::A: return RgbaColor::From(255, 255, 255, pixel[0]);
    }
  ...
  void Pixmap::SetPixel(size_t x, size_t y, RgbaColor color) {
    ...
    case Format::A: pixel[0] = color.alpha; break;
    }
  }

  void Pixmap::ConvertTo(Format newFormat) {
    switch (format) {
    case Format::RGBA: ssassert(newFormat == Format::BGRA, "Unexpected target format"); break;

    case Format::BGRA: ssassert(newFormat == Format::RGBA, "Unexpected target format"); break;

    case Format::RGB: ssassert(newFormat == Format::BGR, "Unexpected target format"); break;

    case Format::BGR: ssassert(newFormat == Format::RGB, "Unexpected target format"); break;

    case Format::A: ssassert(false, "Unexpected target format");
    }
  ...
  static std::shared_ptr<Pixmap> ReadPngIntoPixmap(png_struct *png_ptr, png_info *info_ptr,
    ...
    for (size_t y = 0; y < pixmap->height; y++) {
      ...
      memcpy(&pixmap->data[pixmap->stride * y], srcRow, pixmap->width * pixmap->GetBytesPerPixel());
    }
  ...
  std::shared_ptr<Pixmap> Pixmap::FromPng(const uint8_t *data, size_t size, bool flip) {
    ...
    png_set_read_fn(png_ptr, &dataSlice, [](png_struct *png_ptr, uint8_t *data, size_t size) {
  ...
  bool Pixmap::WritePng(FILE *f, bool flip) {
    ...
    png_init_io(png_ptr, f);
    png_set_IHDR(png_ptr, info_ptr, (png_uint_32)width, (png_uint_32)height, 8, colorType,
...
public:
    ...
    size_t CountUntilEol() const { return std::find(pos, end, '\n') - pos; }
    ...
    uint8_t Read4HexBits() {
      ...
      if (c >= '0' && c <= '9') {
        ...
        ssassert(false, "Unexpected hex digit");
    }

    uint8_t Read8HexBits() {
      uint8_t h = Read4HexBits(), l = Read4HexBits();
      return (h << 4) + l;
    }

    uint16_t Read16HexBits() {
      uint16_t h = Read8HexBits(), l = Read8HexBits();
      return (h << 8) + l;
    }
  ...
  static uint8_t *BitmapFontTextureRow(std::shared_ptr<Pixmap> texture, uint16_t position,
                                       size_t y) {
    // position = 0;
    size_t col = position % (texture->width / 16), row = position / (texture->width / 16);
  ...
  void BitmapFont::AddGlyph(char32_t codepoint, std::shared_ptr<const Pixmap> pixmap) {
    ...
    ssassert(pixmap->format == Pixmap::Format::RGB, "Unexpected pixmap format");
    ssassert(glyphs.find(codepoint) == glyphs.end(), "Glyph with this codepoint already exists");
  ...
  const BitmapFont::Glyph &BitmapFont::GetGlyph(char32_t codepoint) {
    ...
    auto first = unifontData.cbegin(), last = unifontData.cend();
    while (first < last) {
      ...
      if (glyphLength == 4 * 16) {
        ...
        ssassert(false, "Unexpected glyph bitmap length");
  ...
  static void GetGlyphBBox(const VectorFont::Glyph &glyph, double *rminx, double *rmaxx,
  ...
  VectorFont VectorFont::From(std::string &&lffData) {
    ...
    while (reader.TryRegex(std::regex("#\\s*(\\w+)\\s*:\\s*(.+?)\n"), &m)) {
      std::string name = m.str(1), value = m.str(2);
  ...
  const VectorFont::Glyph &VectorFont::GetGlyph(char32_t codepoint) {
    ...
    auto first = firstGlyph, last = lffData.cend();
    while (first <= last) {
      auto mid = first + (last - first) / 2;
      while (mid > first) {
        if (*mid == '[' && *(mid - 1) == '\n')
...
public:
  ...
  int PluralExpr::Token::Precedence() {
    ...
    default: ssassert(false, "Unexpected token op");
    }
  }
  ...
  void PluralExpr::Reduce() {
    ...
    case Token::Type::BINARY_OP: {
      ...
      switch (op.op) {
      case Token::Op::EQ: r.value = (a.value == b.value ? 1 : 0); break;
      case Token::Op::NEQ: r.value = (a.value != b.value ? 1 : 0); break;
      case Token::Op::LT: r.value = (b.value < a.value ? 1 : 0); break;
      case Token::Op::GT: r.value = (b.value > a.value ? 1 : 0); break;
      case Token::Op::LE: r.value = (b.value <= a.value ? 1 : 0); break;
      case Token::Op::GE: r.value = (b.value >= a.value ? 1 : 0); break;
      case Token::Op::AND: r.value = a.value && b.value; break;
      case Token::Op::OR: r.value = a.value || b.value; break;
      case Token::Op::MOD: r.value = b.value % a.value; break;
      case Token::Op::NONE: ssassert(false, "Unexpected operator");
      }
      break;
    }
    ...
    default: ssassert(false, "Unexpected operator type");
    }
  ...
  void PluralExpr::Eval() {
    while (true) {
      ...
      case Token::Type::PAREN_RIGHT:
        while (stack.size() > 1 && stack.end()[-2].type != Token::Type::PAREN_LEFT) {
          Reduce();
        }
      ...
      case Token::Type::VALUE: stack.push_back(t); break;
...
public:
  ...
  void GettextParser::Parse() {
    ...
    while (!reader.AtEnd()) {
      ...
      if (key.ident.empty()) {
        ssassert(msgstrs.size() == 1, "Expected exactly one header msgstr");
        ParseHeader(msgstrs[0]);
      } else {
        ssassert(msgstrs.size() == 1 || msgstrs.size() == pluralCount,
...
public:
  ...
  const std::string &Translation::TranslatePlural(const TranslationKey &key, unsigned n) {
    ...
    if (it->second[pluralForm].empty()) {
      dbp("Missing (empty) translation for %s'%s'[%d]", key.context.c_str(), key.ident.c_str(),
          pluralForm);
  ...
  bool SetLocale(uint16_t lcid) {
    return SetLocale([&](const Locale &locale) { return locale.lcid == lcid; });
  }
