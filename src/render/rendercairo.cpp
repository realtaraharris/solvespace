...
namespace SolveSpace {
  ...
  void CairoRenderer::SelectStroke(hStroke hcs) {
    ...
    cairo_set_source_rgba(context, color.redF(), color.greenF(), color.blueF(), color.alphaF());
    if (antialias) {
  ...
  void CairoRenderer::MoveTo(Vector p) {
    ...
    if (cairo_has_current_point(context) && pos.Equals(p.ProjectXy()))
      return;
    FinishPath();
  ...
  void CairoRenderer::OutputBezier(const SBezier &b, hStroke hcs) {
    ...
    if (b.deg == 1) {
      MoveTo(b.ctrl[0]);
      cairo_line_to(context, b.ctrl[1].x, b.ctrl[1].y);
      ...
      if (dtheta > 0) {
        cairo_arc(context, c.x, c.y, r, theta0, theta1);
      } else {
        cairo_arc_negative(context, c.x, c.y, r, theta0, theta1);
      }
    } else if (b.deg == 3 && !b.IsRational()) {
      MoveTo(b.ctrl[0]);
      cairo_curve_to(context, b.ctrl[1].x, b.ctrl[1].y, b.ctrl[2].x, b.ctrl[2].y, b.ctrl[3].x,
                     b.ctrl[3].y);
    } else {
