...
namespace SolveSpace {
  ...
  void SurfaceRenderer::DrawLine(const Vector &a, const Vector &b, hStroke hcs) {
    edges[hcs].AddEdge(ProjectPoint3RH(camera, a), ProjectPoint3RH(camera, b));
  }

  void SurfaceRenderer::DrawEdges(const SEdgeList &el, hStroke hcs) {
    for (const SEdge &e : el.l) {
      edges[hcs].AddEdge(ProjectPoint3RH(camera, e.a), ProjectPoint3RH(camera, e.b));
    }
  }
  ...
  void SurfaceRenderer::DrawOutlines(const SOutlineList &ol, hStroke hcs, DrawOutlinesAs drawAs) {
    ...
    for (const SOutline &o : ol.l) {
      if (drawAs == DrawOutlinesAs::EMPHASIZED_AND_CONTOUR && !(o.IsVisible(projDir) || o.tag != 0))
        continue;
      if (drawAs == DrawOutlinesAs::EMPHASIZED_WITHOUT_CONTOUR &&
          !(!o.IsVisible(projDir) && o.tag != 0))
        continue;
      if (drawAs == DrawOutlinesAs::CONTOUR_ONLY && !(o.IsVisible(projDir)))
        continue;

      edges[hcs].AddEdge(ProjectPoint3RH(camera, o.a), ProjectPoint3RH(camera, o.b));
    }
  }

  void SurfaceRenderer::DrawVectorText(const std::string &text, double height, const Vector &o,
  ...
  void SurfaceRenderer::DrawQuad(const Vector &a, const Vector &b, const Vector &c, const Vector &d,
    ...
    ssassert(fill->layer == Layer::NORMAL || fill->layer == Layer::DEPTH_ONLY ||
                 fill->layer == Layer::FRONT || fill->layer == Layer::BACK,
             "Unexpected mesh layer");
  ...
  void SurfaceRenderer::DrawPoint(const Vector &o, Canvas::hStroke hcs) {
    ...
    DrawQuad(o.Minus(u).Minus(v), o.Minus(u).Plus(v), o.Plus(u).Plus(v), o.Plus(u).Minus(v), hcf);
  }
  ...
  void SurfaceRenderer::DrawMesh(const SMesh &m, hFill hcfFront, hFill hcfBack) {
  ...
  void SurfaceRenderer::DrawFaces(const SMesh &m, const std::vector<uint32_t> &faces, hFill hcf) {
    ...
    for (STriangle tr : m.l) {
      ...
      for (size_t j = 0; j < facesSize; j++) {
        ...
        mesh.AddTriangle(tr.meta, ProjectPoint3RH(camera, tr.a).Plus(zOffset),
  ...
  void SurfaceRenderer::DrawPixmap(std::shared_ptr<const Pixmap> pm, const Vector &o,
  ...
  void SurfaceRenderer::OutputInPaintOrder() {
    ...
    const Layer stackup[] = {Layer::BACK, Layer::NORMAL, Layer::DEPTH_ONLY, Layer::OCCLUDED,
                             Layer::FRONT};
