...
void Group::Generate(IdList<Entity, hEntity> *entity, IdList<Param, hParam> *param) {
  ...
  case Type::REVOLVE: {
    ...
    for (i = 0; i < entity->n; i++) {
      ...
      CopyEntity(entity, e, ai, REMAP_LATHE_START, h.param(0), h.param(1), h.param(2), h.param(3),
                 h.param(4), h.param(5), h.param(6), NO_PARAM, CopyAs::N_ROT_AA);

      e = &(entity->Get(i)); // because we copied.
      CopyEntity(entity, e, af, REMAP_LATHE_END, h.param(0), h.param(1), h.param(2), h.param(3),
                 h.param(4), h.param(5), h.param(6), NO_PARAM, CopyAs::N_ROT_AA);
  ...
  case Type::HELIX: {
    ...
    for (i = 0; i < entity->n; i++) {
      ...
      CopyEntity(entity, e, ai, REMAP_LATHE_START, h.param(0), h.param(1), h.param(2), h.param(3),
                 h.param(4), h.param(5), h.param(6), h.param(7), CopyAs::N_ROT_AXIS_TRANS);
    ...
    for (a = a0; a < n; a++) {
      ...
      for (i = 0; i < entity->n; i++) {
        ...
        CopyEntity(entity, e, a * 2 - (subtype == Subtype::ONE_SIDED ? 0 : (n - 1)),
                   (a == (n - 1)) ? REMAP_LAST : a, h.param(0), h.param(1), h.param(2), NO_PARAM,
                   NO_PARAM, NO_PARAM, NO_PARAM, NO_PARAM, CopyAs::N_TRANS);
      }
    }
    return;
  }
  case Type::ROTATE: {
    ...
    for (a = a0; a < n; a++) {
      ...
      for (i = 0; i < entity->n; i++) {
        ...
        e->CalculateNumerical(/*forExport=*/false);
        CopyEntity(entity, e, a * 2 - (subtype == Subtype::ONE_SIDED ? 0 : (n - 1)),
                   (a == (n - 1)) ? REMAP_LAST : a, h.param(0), h.param(1), h.param(2), h.param(3),
                   h.param(4), h.param(5), h.param(6), NO_PARAM, CopyAs::N_ROT_AA);
      }
    }
    return;
