...
bool HaikuSpaceUI::OkayToStartNewFile() {
  if (!unsaved) { return true; }
	...
	BAlert *alert = new BAlert(
	  "Modified file", message.c_str(), "Don't save", "Cancel", "Save",
...
void HaikuSpaceUI::GetFilenameAndSave(bool saveAs) {
  if (saveAs || saveFile.IsEmpty()) {
	  BFilePanel *fp = new BFilePanel(
		  ...
		  B_FILE_NODE, false, new BMessage(SAVE_AS_FILE) // calls HaikuSpaceUI::SaveNewFile
		);
    if (!saveFile.IsEmpty()) {
...
bool HaikuSpaceUI::SaveNewFile(Platform::Path newSaveFile) { // called by the SAVE_AS_FILE bMessage handler
  if (!SaveToFile(newSaveFile)) { return false; }
...
bool HaikuSpaceUI::LoadAutosaveFor(const Platform::Path &filename) {
    ...
    fclose(f);
		...
		BAlert *alert = new BAlert(
			"Autosave Available",
			"An autosave file is available for this sketch. Do you want to load the autosave file instead?",
			"Load autosave",
			"Don't load",
...
int HaikuSpaceUI::LocateImportedFile(const Platform::Path &filename, bool canCancel) {
		BAlert *alert = new BAlert(
			"Missing file",
			"The linked file “%s” is not present. Do you want to locate it manually? If you decline, any geometry that depends on the missing file will be permanently removed.",
			"Yes",
			"No",
