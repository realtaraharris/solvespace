...
MainWindow::MainWindow (void)
  ...
  fileMenu->AddItem (
      new BMenuItem ("Export 3D wireframe…", new BMessage (M_EXPORT_WIREFRAME), NULL));
  ...
  constrainMenu->AddItem (new BMenuItem ("On point / curve /plane", new BMessage (M_ON_ENTITY),
                                         'O')); // Command::ON_ENTITY
  constrainMenu->AddItem (new BMenuItem ("Equal length / radius / angle", new BMessage (M_EQUAL),
                                         'Q',
  ...
  case TOGGLE_SNAP_GRID: {
    ...
    if (SS.GW.showSnapGrid && !SS.GW.LockedInWorkplane ()) {
      BAlert *alert = new BAlert ("SolveSpace information dialog",
                                  ...
                                  NULL, "OK", B_WIDTH_AS_USUAL, B_OFFSET_SPACING, B_WARNING_ALERT);
...
void MainWindow::SaveSettings () {
  ...
  if (settings->ReplaceRect ("View Parameters Window",
                             be_app->WindowAt (VIEW_PARAMETERS)->Frame ()) != B_OK) {
