...
void ViewParameters::UpdateViewParameters() {
  overallScaleFactor->SetText(
      std::format("Overall scale factor: {:.3f} px/{}", SS.GW.scale * SS.MmPerUnit(), SS.UnitName())
          .c_str());
  origin->SetText(std::format("({}, {}, {})", SS.MmToString(SS.GW.offset.x),
                              SS.MmToString(SS.GW.offset.y), SS.MmToString(SS.GW.offset.z))
                      .c_str());
  Vector n = (SS.GW.projRight).Cross(SS.GW.projUp);
  projection->SetText(std::format("({:.3f}, {:.3f}, {:.3f})\n({:.3f}, "
                                  "{:.3f}, {:.3f})\n({:.3f}, {:.3f}, {:.3f})",
                                  CO(SS.GW.projRight), CO(SS.GW.projUp), CO(n))
                          .c_str());
  perspectiveFactor->SetText(std::format("{:.3f}", SS.cameraTangent * 1000).c_str());
  explodeDistance->SetText(
      std::format("{} {}", SS.MmToString(SS.explodeDistance), SS.UnitName()).c_str());
}
