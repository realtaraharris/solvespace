...
ThumbListItem::ThumbListItem(BBitmap *thumb, const char *text, int32 w, int level, bool expanded) : BListItem(level,expanded) {
    width = w;
    f_thumb = 0;
...
void ThumbListItem::ChangeIcon(ThumbListItem* item,BBitmap *micon) {
    ...
    if (micon != NULL) {
        ...
        if ((micon->BytesPerRow() == item->icon->BytesPerRow()) && (micon->BitsLength() == item->icon->BitsLength())) {
