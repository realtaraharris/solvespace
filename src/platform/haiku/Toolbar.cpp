...
AppToolbar::AppToolbar(BRect rect, BSize toolbarIconSize)
    ...
    lineToolButton = MakeButton(toolbarIconSize, lineToolIcon,
      new BMessage(LINE_TOOL_BTN_CLICKED), "Sketch line segment (S)");
		rectToolButton = MakeButton(toolbarIconSize, rectToolIcon,
    ...
    circleToolButton = MakeButton(toolbarIconSize, circleToolIcon,
      new BMessage(CIRCLE_TOOL_BTN_CLICKED), "Sketch circle (C)");
		arcToolButton = MakeButton(toolbarIconSize, arcToolIcon,
		  new BMessage(ARC_TOOL_BTN_CLICKED), "Sketch arc (A)");
		tangentArcToolButton = MakeButton(toolbarIconSize, tangentArcToolIcon,
		  new BMessage(TANGENT_ARC_TOOL_BTN_CLICKED), "Create tangent arc at selected point (Shift+A)");
    cubicSplineToolButton = MakeButton(toolbarIconSize, cubicSplineToolIcon,
    ...
    constructionToolButton = MakeButton(toolbarIconSize, constructionToolIcon,
		  new BMessage(CONSTRUCTION_TOOL_BTN_CLICKED), "Toggle construction (G)");
    splitCurvesToolButton = MakeButton(toolbarIconSize, splitCurvesToolIcon,
		  new BMessage(SPLIT_CURVES_TOOL_BTN_CLICKED), "Split intersection of lines and curves (I)");
		textToolButton = MakeButton(toolbarIconSize, textToolIcon,
		  new BMessage(TEXT_TOOL_BTN_CLICKED), "Add curves from text (T)");

    distanceToolButton = MakeButton(toolbarIconSize, distanceToolIcon,
		  new BMessage(DISTANCE_DIA_TOOL_BTN_CLICKED), "Constrain by distance, diameter or length (D)");
    angleToolButton = MakeButton(toolbarIconSize, angleToolIcon,
		  new BMessage(ANGLE_TOOL_BTN_CLICKED), "Constrain by angle (N)");
    horizontalToolButton = MakeButton(toolbarIconSize, horizontalToolIcon,
		  new BMessage(HORIZONTAL_TOOL_BTN_CLICKED), "Constrain horizontally (H)");
		verticalToolButton = MakeButton(toolbarIconSize, verticalToolIcon,
		  new BMessage(VERTICAL_TOOL_BTN_CLICKED), "Constraint vertically (V)");
    parallelToolButton = MakeButton(toolbarIconSize, parallelToolIcon,
		  new BMessage(PARALLEL_TOOL_BTN_CLICKED), "Constrain parallelly (L)");

    constrainPerpendicularToolButton = MakeButton(toolbarIconSize, constrainPerpendicularToolIcon,
    ...
    constrainSymmetricToolButton = MakeButton(toolbarIconSize, constrainSymmetricToolIcon,
    ...
    constrainParallelNormalsToolButton = MakeButton(toolbarIconSize, constrainParallelNormalsToolIcon,
		  new BMessage(CONSTRAIN_ORIENTED_SAME_TOOL_BTN_CLICKED), "Constrain normals in same orientation (X)");
    otherSupplementaryAngleToolButton = MakeButton(toolbarIconSize, otherSupplementaryAngleToolIcon,
		  new BMessage(OTHER_ANGLE_TOOL_BTN_CLICKED), "Find supplementary angle (U)");
    refToolButton = MakeButton(toolbarIconSize, refToolIcon,
		  new BMessage(REF_TOOL_BTN_CLICKED), "Toggle reference dimension (E)");

    nearestIsoToolButton = MakeButton(toolbarIconSize, nearestIsoToolIcon,
