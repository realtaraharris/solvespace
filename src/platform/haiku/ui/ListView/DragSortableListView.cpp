...
DragSortableListView::DragSortableListView(BRect frame, const char *name,
    ...
    : BListView(frame, name, type, resizingMode, flags),
      fDropRect(0.0, 0.0, -1.0, -1.0), fMouseWheelFilter(NULL),
      fScrollPulse(NULL), fDropIndex(-1), fLastClickedItem(NULL),
...
void DragSortableListView::MessageReceived(BMessage *message) {
    if (message->what == fDragCommand) {
            ...
            if (scrollV != 0.0 && fScrollView) {
                if (BScrollBar *scrollBar = fScrollView->ScrollBar(B_VERTICAL)) {
                    ...
                    if (scrollBar->Value() != value) {
                        ...
                        BPoint point;
                        GetMouse(&point, &buttons, false);
                        uint32 transit = Bounds().Contains(point)
                                             ? B_INSIDE_VIEW
                                             : B_OUTSIDE_VIEW;
                        MouseMoved(point, transit, &fDragMessageCopy);
                    }
        ...
        case B_MOUSE_WHEEL_CHANGED: {
            ...
            GetMouse(&point, &buttons, false);
            uint32 transit =
                Bounds().Contains(point) ? B_INSIDE_VIEW : B_OUTSIDE_VIEW;
            MouseMoved(point, transit, &fDragMessageCopy);
            break;
        }
        default:
...
void DragSortableListView::MouseDown(BPoint where) {
    ...
    if (ListType() == B_MULTIPLE_SELECTION_LIST && item &&
...
bool DragSortableListView::DoesAutoScrolling() const { return fScrollPulse; }
...
void DragSortableListView::CopyItems(BList &items, int32 index) {
    ...
    for (int32 i = 0; BListItem *item = (BListItem *)clonedItems.ItemAt(i);
         i++) {
        if (AddItem(item, index)) {
            // after we're done, the newly inserted items will be selected
            Select(index, true);
...
void DragSortableListView::SelectAll() { Select(0, CountItems() - 1); }
...
SimpleListView::SimpleListView(BRect frame, BMessage *selectionChangeMessage)
    : DragSortableListView(
          frame, "playlist listview", B_MULTIPLE_SELECTION_LIST, B_FOLLOW_ALL,
...
SimpleListView::~SimpleListView() { delete fSelectionChangeMessage; }
...
void SimpleListView::MakeDragMessage(BMessage *message) const {
    if (message) {
        message->AddPointer(
            "list", (void *)dynamic_cast<const DragSortableListView *>(this));
        int32 index;
