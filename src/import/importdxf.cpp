...
public:
    ...
    void multBlockTransform (double x, double y, double sx, double sy, double angle) {
      ...
      blockX = oldX.ScaledBy (newX.x).Plus (oldY.ScaledBy (newX.y));

      blockY = oldX.ScaledBy (newY.x).Plus (oldY.ScaledBy (newY.y));

      blockT = oldX.ScaledBy (newT.x).Plus (oldY.ScaledBy (newT.y)).Plus (oldT);
    }
    ...
    void addEntity (DRW_Entity *e) {
      switch (e->eType) {
      case DRW::POINT: addPoint (*static_cast<DRW_Point *> (e)); break;
      case DRW::LINE: addLine (*static_cast<DRW_Line *> (e)); break;
      case DRW::ARC: addArc (*static_cast<DRW_Arc *> (e)); break;
      case DRW::CIRCLE: addCircle (*static_cast<DRW_Circle *> (e)); break;
      case DRW::POLYLINE: addPolyline (*static_cast<DRW_Polyline *> (e)); break;
      case DRW::LWPOLYLINE: addLWPolyline (*static_cast<DRW_LWPolyline *> (e)); break;
      case DRW::SPLINE: addSpline (static_cast<DRW_Spline *> (e)); break;
      case DRW::INSERT: addInsert (*static_cast<DRW_Insert *> (e)); break;
      case DRW::TEXT: addText (*static_cast<DRW_Text *> (e)); break;
      case DRW::MTEXT: addMText (*static_cast<DRW_MText *> (e)); break;
      case DRW::DIMALIGNED: addDimAlign (static_cast<DRW_DimAligned *> (e)); break;
      case DRW::DIMLINEAR: addDimLinear (static_cast<DRW_DimLinear *> (e)); break;
      case DRW::DIMRADIAL: addDimRadial (static_cast<DRW_DimRadial *> (e)); break;
      case DRW::DIMDIAMETRIC: addDimDiametric (static_cast<DRW_DimDiametric *> (e)); break;
      case DRW::DIMANGULAR: addDimAngular (static_cast<DRW_DimAngular *> (e)); break;
      default: unknownEntities++;
      }
    }
    ...
    Style::TextOrigin dxfAlignToOrigin (DRW_Text::HAlign alignH, DRW_Text::VAlign alignV) {
      uint32_t origin = 0;
      switch (alignH) {
      case DRW_Text::HLeft: origin |= (uint32_t)Style::TextOrigin::LEFT; break;

      case DRW_Text::HMiddle:
      case DRW_Text::HCenter: break;

      case DRW_Text::HRight: origin |= (uint32_t)Style::TextOrigin::RIGHT; break;

      case DRW_Text::HAligned:
      case DRW_Text::HFit:
      default: origin |= (uint32_t)Style::TextOrigin::LEFT; break;
      }

      switch (alignV) {
      case DRW_Text::VBaseLine:
      case DRW_Text::VBottom: origin |= (uint32_t)Style::TextOrigin::BOT; break;

      case DRW_Text::VMiddle: break;

      case DRW_Text::VTop: origin |= (uint32_t)Style::TextOrigin::TOP; break;

      default: origin |= (uint32_t)Style::TextOrigin::BOT; break;
      }
    ...
    struct VectorPred {
      bool operator() (Vector a, Vector b) const { return a.Equals (b, LENGTH_EPS); }
    };
    ...
    void endBlock () override { readBlock = NULL; }

    void addPoint (const DRW_Point &data) override {
      if (data.space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_Point> (data))
        return;
    ...
    void addLine (const DRW_Line &data) override {
      if (data.space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_Line> (data))
        return;
    ...
    void addLWPolyline (const DRW_LWPolyline &data) override {
      if (data.space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_LWPolyline> (data))
        return;
    ...
    void addPolyline (const DRW_Polyline &data) override {
      if (data.space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_Polyline> (data))
        return;
    ...
    void addSpline (const DRW_Spline *data) override {
      ...
      if (data->degree != 3)
        return;
      if (addPendingBlockEntity<DRW_Spline> (*data))
        return;
    ...
    void addInsert (const DRW_Insert &data) override {
      if (data.space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_Insert> (data))
        return;
    ...
    void addMText (const DRW_MText &data) override {
      if (data.space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_MText> (data))
        return;
    ...
    void addText (const DRW_Text &data) override {
      if (data.space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_Text> (data))
        return;
    ...
    void addDimAlign (const DRW_DimAligned *data) override {
      ...
      hConstraint hc =
          Constraint::Constrain (Constraint::Type::PT_PT_DISTANCE, createOrGetPoint (p0),
    ...
    void addDimLinear (const DRW_DimLinear *data) override {
      if (data->space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_DimLinear> (*data))
        return;
      ...
      hConstraint hc =
          Constraint::Constrain (Constraint::Type::PT_LINE_DISTANCE, createOrGetPoint (p0),
                                 Entity::NO_ENTITY, createLine (p1, p3, invisibleStyle ()));
    ...
    void addDimAngular (const DRW_DimAngular *data) override {
      if (data->space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_DimAngular> (*data))
        return;
      ...
      hConstraint hc = Constraint::Constrain (
          Constraint::Type::ANGLE, Entity::NO_ENTITY, Entity::NO_ENTITY,
          createLine (l0p0, l0p1, invisibleStyle ()), createLine (l1p1, l1p0, invisibleStyle ()),
          /*other=*/false,
          /*other2=*/false);
      ...
      VectorAtIntersectionOfLines_ret eeep =
          VectorAtIntersectionOfLines (l0p0, l0p1, l1p0, l1p1, false);
      if (!eeep.skewed) {
        c->disp.offset = toVector (data->getTextPoint ()).Minus (eeep.intersectionPoint);
      }
    }

    hConstraint createDiametric (Vector cp, Quaternion q, double r, Vector tp, double actual,
                                 bool asRadius = false) {
      hEntity he = createCircle (cp, q, r, invisibleStyle ());

      hConstraint hc = Constraint::Constrain (Constraint::Type::DIAMETER, Entity::NO_ENTITY,
                                              Entity::NO_ENTITY, he);
    ...
    void addDimAngular3P (const DRW_DimAngular3p *data) override {
      if (data->space != DRW::ModelSpace)
        return;
      if (addPendingBlockEntity<DRW_DimAngular3p> (*data))
        return;
...
public:
  ...
  ImportDwgDxf (const Platform::Path                                                     &filename,
    ...
    if (importer.unknownEntities > 0) {
      Message ("%u %s entities of unknown type were ignored.", importer.unknownEntities,
               fileType.c_str ());
    }
  }
