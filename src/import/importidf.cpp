...
static void CreateEntity (EntityList *el, int *id, hEntity h0, hEntity h1, hEntity hnorm, Vector p0,
                          Vector p1, double angle, bool keepout) {
  if (angle == 0.0) {
    // line
    if (p0.Equals (p1))
      return;
...
static void MakeBeziersForArcs (SBezierList *sbl, Vector center, Vector pa, Vector pb, Quaternion q,
                                double angle) {
...
namespace SolveSpace {
  ...
  bool LinkIDF (const Platform::Path &filename, EntityList *el, SMesh *m, SShell *sh) {
    ...
    for (std::string line; getline (stream, line);) {
      ...
      case notes:
      case component_placement: break;

      default: section = none; break;
      }
      record_number++;
    }
    ...
    sh->MakeFromExtrusionOf (sblss.l.First (), Vector::From (0.0, 0.0, 0.0),
                             Vector::From (0.0, 0.0, board_thickness), RgbaColor::From (0, 180, 0));
