...
bool SShell::CheckNormalAxisRelationship(SBezierLoopSet *sbls, Vector pt, Vector axis, double da,
                                         double dx)
...
// the sketch, on the side of the axis where the sketch is.
{
  ...
  Vector ptc = pto.ClosestPointOnLine(pt, axis), up = axis.Cross(pto.Minus(ptc)).ScaledBy(da),
  ...
  for (sbl = sbls->l.First(); sbl; sbl = sbls->l.NextAfter(sbl)) {
    ...
    for (sb = sbl->l.First(); sb; sb = sbl->l.NextAfter(sb)) {
      ...
      for (j = 0; j < sections; j++) {
        if ((dist == 0) && sb->deg == 1 && (sb->ctrl[0]).DistanceToLine(pt, axis) < LENGTH_EPS &&
    ...
    for (i = 0; i < sbl->l.n; i++) {
      ...
      if (revs[0].v) {
        double ps = 0.0;
        t_values.Add(&ps);
        (surface.FindById(revs[0]))->MakeTriangulationGridInto(&t_values, 0.0, 1.0, true, 0);
      }
...
void SShell::MakeFromRevolutionOf(SBezierLoopSet *sbls, Vector pt, Vector axis, RgbaColor color,
  ...
  for (sbl = sbls->l.First(); sbl; sbl = sbls->l.NextAfter(sbl)) {
    ...
    for (sb = sbl->l.First(); sb; sb = sbl->l.NextAfter(sb)) {
      ...
      for (j = 0; j < 4; j++) {
        if (sb->deg == 1 && (sb->ctrl[0]).DistanceToLine(pt, axis) < LENGTH_EPS &&
            (sb->ctrl[1]).DistanceToLine(pt, axis) < LENGTH_EPS) {
          // This is a line on the axis of revolution; it does
          // not contribute a surface.
          revs[j].v = 0;
          ...
          SSurface ss =
              SSurface::FromRevolutionOf(sb, pt, axis, (PI / 2) * j, (PI / 2) * (j + 1), 0.0, 0.0);
          ss.color = color;
          if (sb->entity != 0) {
            hEntity he;
            he.v = sb->entity;
...
void SShell::MakeFirstOrderRevolvedSurfaces(Vector pt, Vector axis, int i0) {
  ...
  for (i = i0; i < surface.n; i++) {
    ...
    if (srf->degm == 1 && srf->degn == 2) {
      ...
      Vector oc = cs.ClosestPointOnLine(pt, axis), of = fs.ClosestPointOnLine(pt, axis);
