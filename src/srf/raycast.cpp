...
void SSurface::BlendRowOrCol(bool row, int this_ij, SSurface *a, int a_ij, SSurface *b, int b_ij) {
  if (row) {
    int j;
    for (j = 0; j <= degn; j++) {
...
//-----------------------------------------------------------------------------
void SSurface::AllPointsIntersectingUntrimmed(Vector a, Vector b, int *cnt, int *level,
                                              List<Inter> *l, bool asSegment, SSurface *sorig) {
  ...
  if (DepartureFromCoplanar() < 0.2 * SS.ChordTolMm()) {
    Vector p =
        (ctrl[0][0]).Plus(ctrl[0][degn]).Plus(ctrl[degm][0]).Plus(ctrl[degm][degn]).ScaledBy(0.25);
    Inter inter;
...
//-----------------------------------------------------------------------------
void SSurface::AllPointsIntersecting(Vector a, Vector b, List<SInter> *l, bool asSegment,
                                     bool trimmed, bool inclTangent) {
  if (LineEntirelyOutsideBbox(a, b, asSegment))
    return;
...
void SShell::AllPointsIntersecting(Vector a, Vector b, List<SInter> *il, bool asSegment,
                                   bool trimmed, bool inclTangent) {
  for (SSurface &ss : surface) {
    ss.AllPointsIntersecting(a, b, il, asSegment, trimmed, inclTangent);
  }
}

SShell::Class SShell::ClassifyRegion(Vector edge_n, Vector inter_surf_n, Vector edge_surf_n) const {
...
bool SShell::ClassifyEdge(Class *indir, Class *outdir, Vector ea, Vector eb, Vector p,
    ...
    for (se = sel->l.First(); se; se = sel->l.NextAfter(se)) {
      if ((ea.Equals(se->a) && eb.Equals(se->b)) || (eb.Equals(se->a) && ea.Equals(se->b)) ||
        ...
        if (edge_inters < 2) {
          ...
          // out.
          inter_edge_n[edge_inters] = (inter_surf_n[edge_inters]).Cross((se->b).Minus((se->a)));
        }
  ...
  if (edge_inters == 2) {
    ...
    if (fabs(dotp[1]) < DOTP_TOL) {
      std::swap(dotp[0], dotp[1]);
      std::swap(inter_surf_n[0], inter_surf_n[1]);
      std::swap(inter_edge_n[0], inter_edge_n[1]);
    }
  ...
  if (edge_inters != 0)
    dbp("bad, edge_inters=%d", edge_inters);
