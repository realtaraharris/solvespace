  ...
  for (;;) {
    ...
    do {
      ...
      for (se = el->l.First(); se; se = el->l.NextAfter(se)) {
        if (se->tag)
          continue;
        if (se->auxA != (int)stb.curve.v)
          continue;
        if ((se->auxB && !stb.backwards) || (!se->auxB && stb.backwards))
          continue;
...
static bool KeepRegion(SSurface::CombineAs type, bool opA, SShell::Class shell,
                       SShell::Class orig) {
  bool inShell = (shell == SShell::Class::INSIDE), outSide = (shell == SShell::Class::OUTSIDE),
       inSame = (shell == SShell::Class::COINC_SAME), inOrig = (orig == SShell::Class::INSIDE);
...
static void TagByClassifiedEdge(SBspUv::Class bspclass, SShell::Class *indir,
                                SShell::Class *outdir) {
  switch (bspclass) {
...
static void DEBUGEDGELIST(SEdgeList *sel, SSurface *surf) {
  ...
  for (se = sel->l.First(); se; se = sel->l.NextAfter(se)) {
    ...
    SS.nakedEdges.AddEdge(surf->PointAt(se->a.x, se->a.y), surf->PointAt(se->b.x, se->b.y));
    SS.nakedEdges.AddEdge(surf->PointAt(mid.x, mid.y), surf->PointAt(arrow.x, arrow.y));
  }
}
...
void SSurface::FindChainAvoiding(SEdgeList *src, SEdgeList *dest, SPointList *avoid) {
  ...
  do {
    ...
    Vector s = dest->l.First()->a, f = dest->l.Last()->b;
    ...
    bool startOkay = !avoid->ContainsPoint(s), finishOkay = !avoid->ContainsPoint(f);
...
void SSurface::EdgeNormalsWithinSurface(Point2d auv, Point2d buv, Vector *pt, Vector *enin,
                                        Vector *enout, Vector *surfn, uint32_t auxA, SShell *shell,
  ...
  *pt = PointAt(muv);
  ...
  if (sc->isExact && sc->exact.deg != 1) {
    ...
    *pt = sc->exact.PointAt(t);
    ...
    SSurface *trimmedA = sc->GetSurfaceA(sha, shb), *trimmedB = sc->GetSurfaceB(sha, shb);
  ...
  *surfn = NormalAt(muv.x, muv.y);
  ...
  // surface is curved.
  Vector pin = PointAt(muv.Minus(enuv)), pout = PointAt(muv.Plus(enuv));
  *enin = pin.Minus(*pt), *enout = pout.Minus(*pt);
}
...
SSurface SSurface::MakeCopyTrimAgainst(SShell *parent, SShell *sha, SShell *shb, SShell *into,
  ...
  for (SCurve &sc : into->curve) {
    ...
    for (i = 1; i < sc.pts.n; i++) {
      Vector a = sc.pts[i - 1].p, b = sc.pts[i].p;
      ...
      if (c != SBspUv::Class::OUTSIDE) {
        ...
        if ((tn.Cross(b.Minus(a))).Dot(sn) < 0)
          bkwds = !bkwds;
        if ((type == SSurface::CombineAs::DIFFERENCE && !opA) ||
            (type == SSurface::CombineAs::INTERSECTION)) { // Invert all newly created edges for
                                                           // intersection
          bkwds = !bkwds;
        }
        if (bkwds) {
  ...
  while (!orig.l.IsEmpty()) {
    ...
    Point2d auv = (se->a).ProjectXy(), buv = (se->b).ProjectXy();

    Vector pt, enin, enout, surfn;
    ret.EdgeNormalsWithinSurface(auv, buv, &pt, &enin, &enout, &surfn, se->auxA, into, sha, shb);
    ...
    if (KeepEdge(type, opA, indir_shell, outdir_shell, indir_orig, outdir_orig)) {
  ...
  while (!inter.l.IsEmpty()) {
    ...
    SBspUv::Class c_this =
        (origBsp) ? origBsp->ClassifyEdge(auv, buv, &ret) : SBspUv::Class::OUTSIDE;
...
double SBspUv::ScaledSignedDistanceToLine(Point2d pt, Point2d a, Point2d b, SSurface *srf) const {
