...
bool SSurface::IsCylinder(Vector *axis, Vector *center, double *r, Vector *start,
                          Vector *finish) const {
  SBezier sb;
  if (!IsExtrusion(&sb, axis))
    return false;
...
SSurface SSurface::FromRevolutionOf(SBezier *sb, Vector pt, Vector axis, double thetas,
  ...
  for (i = 0; i <= ret.degm; i++) {
    ...
    Vector ps = p.RotatedAbout(pt, axis, thetas), pf = p.RotatedAbout(pt, axis, thetaf);
...
void SSurface::MakeTrimEdgesInto(SEdgeList *sel, MakeAs flags, SCurve *sc, STrimBy *stb) {
  ...
  for (i = first; i != (last + increment); i += increment) {
    ...
    if (pt->Equals(stb->start))
      inCurve = true;
    if (pt->Equals(stb->finish))
      inCurve = false;
  }
  if (inCurve)
    dbp("trim was unterminated");
  if (empty)
    dbp("trim was empty");
}
...
// shell.
//-----------------------------------------------------------------------------
void SSurface::MakeEdgesInto(SShell *shell, SEdgeList *sel, MakeAs flags, SShell *useCurvesFrom) {
  STrimBy *stb;
  ...
  for (stb = trim.First(); stb; stb = trim.NextAfter(stb)) {
    ...
    if (sbl && sc->isExact && (sb->deg != 1 || !sel)) {
      ...
      for (sp = 0; sp < sc->pts.n; sp++) {
        if (s.Equals(sc->pts[sp].p))
          break;
      }
      if (sp >= sc->pts.n)
        return;
      for (fp = sp; fp < sc->pts.n; fp++) {
        if (f.Equals(sc->pts[fp].p))
          break;
      }
      if (fp >= sc->pts.n)
        return;
      ...
      while (sp < fp) {
        ...
        for (;;) {
          ...
          Vector st = sc->pts[sp].p, ft = sc->pts[fpt].p, sf = ft.Minus(st);
          double m = sf.Magnitude() / 3;

          Vector stan = ExactSurfaceTangentAt(st, srfA, srfB, sf),
                 ftan = ExactSurfaceTangentAt(ft, srfA, srfB, sf);

          SBezier sb = SBezier::From(st, st.Plus(stan.WithMagnitude(m)),
                                     ft.Minus(ftan.WithMagnitude(m)), ft);
      ...
      if (sel)
        MakeTrimEdgesInto(sel, MakeAs::XYZ, sc, stb);
    }
  }
}
