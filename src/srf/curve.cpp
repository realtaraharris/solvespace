...
SBezier SBezier::From(Vector p0, Vector p1) {
  return SBezier::From(p0.Project4d(), p1.Project4d());
}

SBezier SBezier::From(Vector p0, Vector p1, Vector p2) {
  return SBezier::From(p0.Project4d(), p1.Project4d(), p2.Project4d());
}

SBezier SBezier::From(Vector p0, Vector p1, Vector p2, Vector p3) {
  return SBezier::From(p0.Project4d(), p1.Project4d(), p2.Project4d(), p3.Project4d());
}
...
void SBezier::Reverse() {
  int i;
  for (i = 0; i < (deg + 1) / 2; i++) {
    std::swap(ctrl[i], ctrl[deg - i]);
    std::swap(weight[i], weight[deg - i]);
  }
}
...
void SBezier::GetBoundingProjd(Vector u, Vector orig, double *umin, double *umax) const {
  int i;
  for (i = 0; i <= deg; i++) {
...
bool SBezier::IsCircle(Vector axis, Vector *center, double *r) const {
  ...
  Vector t0 = (ctrl[0]).Minus(ctrl[1]), t2 = (ctrl[2]).Minus(ctrl[1]), r0 = axis.Cross(t0),
         r2 = axis.Cross(t2);

  VectorAtIntersectionOfLines_ret eeep =
      VectorAtIntersectionOfLines(ctrl[0], (ctrl[0]).Plus(r0), ctrl[2], (ctrl[2]).Plus(r2), false);
  *center = eeep.intersectionPoint;

  double rd0 = center->Minus(ctrl[0]).Magnitude(), rd2 = center->Minus(ctrl[2]).Magnitude();
  ...
  *r = rd0;
  ...
  double thetaa = atan2(pa2.y, pa2.x), // in fact always zero due to csys
      thetab = atan2(pb2.y, pb2.x), dtheta = WRAP_NOT_0(thetab - thetaa, 2 * PI);
  if (dtheta > PI) {
...
bool SBezier::Equals(SBezier *b) const {
  ...
  int i;
  for (i = 0; i <= deg; i++) {
    if (!(ctrl[i]).Equals(b->ctrl[i]))
      return false;
    if (fabs(weight[i] - b->weight[i]) > LENGTH_EPS)
      return false;
  }
  return true;
}
...
//-----------------------------------------------------------------------------
SBezierLoop SBezierLoop::FromCurves(SBezierList *sbl, bool *allClosed, SEdge *errorAt) {
  SBezierLoop loop = {};
...
void SBezierLoop::GetBoundingProjd(Vector u, Vector orig, double *umin, double *umax) const {
  for (const SBezier *sb = l.First(); sb; sb = l.NextAfter(sb)) {
...
bool SBezierLoop::IsClosed() const {
  if (l.IsEmpty())
    return false;
  Vector s = l.First()->Start(), f = l.Last()->Finish();
  return s.Equals(f);
}
...
//-----------------------------------------------------------------------------
SBezierLoopSet SBezierLoopSet::From(SBezierList *sbl, SPolygon *poly, double chordTol,
                                    bool *allClosed, SEdge *errorAt, SBezierLoopSet *openContours) {
  SBezierLoopSet ret = {};
...
void SBezierLoopSet::GetBoundingProjd(Vector u, Vector orig, double *umin, double *umax) const {
...
//-----------------------------------------------------------------------------
void SBezierLoopSetSet::FindOuterFacesFrom(SBezierList *sbl, SPolygon *spxyz, SSurface *srfuv,
                                           double chordTol, bool *allClosed, SEdge *notClosedAt,
                                           bool *allCoplanar, Vector *notCoplanarAt,
                                           SBezierLoopSet *openContours) {
  SSurface srfPlane;
  if (!srfuv) {
    Vector p, u, v;
    *allCoplanar = sbl->GetPlaneContainingBeziers(&p, &u, &v, notCoplanarAt);
    if (!*allCoplanar) {
  ...
  SBezierLoopSet sbls =
      SBezierLoopSet::From(sbl, spxyz, chordTol, allClosed, notClosedAt, openContours);
  if (sbls.l.n != spxyz->l.n)
    return;
  ...
  while (loopsRemaining) {
    ...
    for (i = 0; i < sbls.l.n; i++) {
      ...
      for (j = 0; j < sbls.l.n; j++) {
        ...
        if (inner->tag != INNER_LOOP)
          continue;
        if (inner->l.n < 1)
          continue;
        if (inner->l[0].auxA != auxA)
          continue;
...
SSurface *SCurve::GetSurfaceA(SShell *a, SShell *b) const {
  if (source == Source::A) {
    ...
    ssassert(false, "Unexpected curve source");
}
...
void SCurve::RemoveShortSegments(SSurface *srfA, SSurface *srfB) {
  ...
  int i, a;
  for (i = 1; i < pts.n - 1; i++) {
    SCurvePt *sct = &(pts[i]), *scn = &(pts[i + 1]);
