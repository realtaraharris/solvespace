...
bool SContour::IsEmptyTriangle(int ap, int bp, int cp, double scaledEPS) const {
  ...
  for (i = 0; i < l.n; i++) {
    ...
    Vector p = l[i].p;
    if (p.OutsideAndNotOn(maxv, minv))
      continue;
    ...
    if (p.EqualsExactly(tr.a))
      continue;
    if (p.EqualsExactly(tr.b))
      continue;
    if (p.EqualsExactly(tr.c))
      continue;
...
bool SContour::IsEar(int bp, double scaledEps) const {
  ...
  for (i = 0; i < l.n; i++) {
    ...
    if (p.EqualsExactly(tr.b)) {
      ...
      if (jp.Equals(tr.c) && kp.Equals(tr.a))
        return false;
...
void SContour::UvTriangulateInto(SMesh *m, SSurface *srf) {
  ...
  if (srf->degm == 1 && srf->degn == 1) {
    ...
    for (i = 1; i < l.n; i++) {
      ...
      if (!IsEar(i - 1, scaledEps))
        end = true;
      //            if ((j>0) && !IsEar(pstart, i-1, i, scaledEps)) end = true;
      if ((j > 0) && !IsEmptyTriangle(pstart, i - 1, i, scaledEps))
        end = true;
...
void SSurface::MakeTriangulationGridInto(List<double> *l, double vs, double vf, bool swapped,
                                         int depth) const {
  double worst = 0;
  ...
  for (i = 0; i <= 3; i++) {
    ...
    // less.
    Vector ps = PointAtMaybeSwapped(u, vs, swapped), pf = PointAtMaybeSwapped(u, vf, swapped);

    double vm1 = (2 * vs + vf) / 3, vm2 = (vs + 2 * vf) / 3;

    Vector pm1 = PointAtMaybeSwapped(u, vm1, swapped), pm2 = PointAtMaybeSwapped(u, vm2, swapped);
    ...
    double twist = 1.0;
    if (degm == 1)
      twist = NormalAtMaybeSwapped(u, vm1, swapped).Dot(NormalAtMaybeSwapped(u, vm2, swapped));
  ...
  if (((vf - vs) < step || worst < SS.ChordTolMm()) && ((worst_twist > 0.999) || (depth > 3))) {
    l->Add(&vf);
  } else {
  ...
  if ((li.n > 3) && (lj.n > 3)) {
    ...
    for (i = 1; i < (li.n - 1); i++) {
      ...
      for (j = 1; j < (lj.n - 1); j++) {
        ...
        if ((i == (li.n - 2)) || (j == (lj.n - 2)) || orig.AnyEdgeCrossings(a, b, NULL) ||
            orig.AnyEdgeCrossings(b, c, NULL) || orig.AnyEdgeCrossings(c, d, NULL) ||
