...
void SSurface::AddExactIntersectionCurve(SBezier *sb, SSurface *srfB, SShell *agnstA,
                                         SShell *agnstB, SShell *into) {
  SCurve sc = {};
...
#endif // 0
  ssassert(!(sb->Start()).Equals(sb->Finish()), "Unexpected zero-length edge");
...
void SSurface::IntersectAgainst(SSurface *b, SShell *agnstA, SShell *agnstB, SShell *into) {
  ...
  if (degm == 1 && degn == 1 && b->degm == 1 && b->degn == 1) {
    ...
    Vector na = NormalAt(0, 0).WithMagnitude(1), nb = b->NormalAt(0, 0).WithMagnitude(1);
    double da = na.Dot(PointAt(0, 0)), db = nb.Dot(b->PointAt(0, 0));
    ...
    if (tmax > tmin + LENGTH_EPS) {
      SBezier bezier = SBezier::From(p.Plus(dl.ScaledBy(tmin)), p.Plus(dl.ScaledBy(tmax)));
      AddExactIntersectionCurve(&bezier, b, agnstA, agnstB, into);
    }
  } else if ((degm == 1 && degn == 1 && isExtdb) || (b->degm == 1 && b->degn == 1 && isExtdt)) {
    ...
    if (fabs(n.Dot(along)) < LENGTH_EPS) {
      ...
      for (i = 0; i <= bezier.deg; i++) {
        Vector p0 = bezier.ctrl[i], p1 = p0.Plus(along);

        bezier.ctrl[i] = VectorAtIntersectionOfPlaneAndLine(n, d, p0, p1, NULL);
      }
    ...
    if (a_axis0 > a_axis1)
      std::swap(a_axis0, a_axis1);
    if (b_axis0 > b_axis1)
      std::swap(b_axis0, b_axis1);

    double ab_axis0 = std::max(a_axis0, b_axis0), ab_axis1 = std::min(a_axis1, b_axis1);
    ...
    Vector axis0 = axis.ScaledBy(ab_axis0), axis1 = axis.ScaledBy(ab_axis1),
    ...
    if ((degm == 1 && degn == 1) || (b->degm == 1 && b->degn == 1)) {
      ...
      for (SCurve &sc : shext->curve) {
        if (sc.source == SCurve::Source::INTERSECTION)
          continue;
        if (!sc.isExact)
          continue;
        if ((sc.surfA != sext->h) && (sc.surfB != sext->h))
          continue;
    ...
    while (spl.l.n >= 2) {
      ...
      for (a = 0; a < maxsteps; a++) {
        ...
        Vector na = NormalAt(pa).WithMagnitude(1), nb = b->NormalAt(pb).WithMagnitude(1);
...
bool SSurface::CoincidentWithPlane(Vector n, double d) const {
  if (degm != 1 || degn != 1)
    return false;
  if (fabs(n.Dot(ctrl[0][0]) - d) > LENGTH_EPS)
    return false;
  if (fabs(n.Dot(ctrl[0][1]) - d) > LENGTH_EPS)
    return false;
  if (fabs(n.Dot(ctrl[1][0]) - d) > LENGTH_EPS)
    return false;
  if (fabs(n.Dot(ctrl[1][1]) - d) > LENGTH_EPS)
    return false;
...
bool SSurface::ContainsPlaneCurve(SCurve *sc) const {
  if (degm != 1 || degn != 1)
    return false;
  if (!sc->isExact)
    return false; // we don't handle those (yet?)
  ...
  for (int i = 0; i <= sc->exact.deg; i++) {
    if (fabs(n.Dot(sc->exact.ctrl[i]) - d) > LENGTH_EPS)
      return false;
  }
  return true;
}
...
//-----------------------------------------------------------------------------
void SShell::MakeCoincidentEdgesInto(SSurface *proto, bool sameNormal, SEdgeList *el,
