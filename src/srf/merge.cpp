  ...
  for (i = 0; i < surface.n; i++) {
    ...
    do {
      ...
      for (j = i + 1; j < surface.n; j++) {
        ...
        if (sj->tag)
          continue;
        if (!sj->CoincidentWith(si, /*sameNormal=*/true))
          continue;
        if (!sj->color.Equals(si->color))
          continue;
    ...
    if (merged) {
      ...
      double umax = VERY_NEGATIVE, umin = VERY_POSITIVE, vmax = VERY_NEGATIVE, vmin = VERY_POSITIVE;
      SEdge *se;
      ...
      si->ctrl[1][0] = Vector::From(umax, vmin, nt).ScaleOutOfCsys(u, v, n);
    }
    sel.Clear();
  }
