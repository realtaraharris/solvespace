...
void Constraint::DoEqualLenTicks (Canvas *canvas, Canvas::hStroke hcs, Vector a, Vector b,
                                  Vector gn, Vector *refp) {
  const Camera &camera = canvas->GetCamera ();
...
void Constraint::DoEqualRadiusTicks (Canvas *canvas, Canvas::hStroke hcs, hEntity he,
                                     Vector *refp) {
  const Camera &camera = canvas->GetCamera ();
...
void Constraint::DoArcForAngle (Canvas *canvas, Canvas::hStroke hcs, Vector a0, Vector da,
                                Vector b0, Vector db, Vector offset, Vector *ref, bool trim,
...
void Constraint::DoLayout (DrawAs how, Canvas *canvas, Vector *labelPos,
  ...
  case Type::WHERE_DRAGGED: {
    ...
    if (refs)
      refs->push_back (p);
    Vector u = p.Plus (gu.WithMagnitude (8 / camera.scale))
                   .Plus (gr.WithMagnitude (8 / camera.scale)),
