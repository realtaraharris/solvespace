...
std::string Constraint::Label() const {
  ...
  } else if (type == Type::LENGTH_RATIO || type == Type::ARC_ARC_LEN_RATIO ||
             type == Type::ARC_LINE_LEN_RATIO) {
...
void Constraint::DoLabel(Canvas *canvas, Canvas::hStroke hcs, Vector ref, Vector *labelPos,
                         Vector gr, Vector gu) {
  const Camera &camera = canvas->GetCamera();
  ...
  if (type == Type::COMMENT && disp.style.v) {
    ...
    if (o & (uint32_t)Style::TextOrigin::LEFT)
      ref = ref.Plus(gr.WithMagnitude(swidth / 2));
    if (o & (uint32_t)Style::TextOrigin::RIGHT)
      ref = ref.Minus(gr.WithMagnitude(swidth / 2));
    if (o & (uint32_t)Style::TextOrigin::BOT)
      ref = ref.Plus(gu.WithMagnitude(sheight / 2));
    if (o & (uint32_t)Style::TextOrigin::TOP)
      ref = ref.Minus(gu.WithMagnitude(sheight / 2));
  }

  Vector o = ref.Minus(gr.WithMagnitude(swidth / 2)).Minus(gu.WithMagnitude(sheight / 2));
  canvas->DrawVectorText(s, textHeight, o, gr.WithMagnitude(1), gu.WithMagnitude(1), hcs);
  if (labelPos)
    *labelPos = o;
}

void Constraint::DoProjectedPoint(Canvas *canvas, Canvas::hStroke hcs, Vector *r, Vector n,
                                  Vector o) {
...
//-----------------------------------------------------------------------------
int Constraint::DoLineTrimmedAgainstBox(Canvas *canvas, Canvas::hStroke hcs, Vector ref, Vector a,
                                        Vector b, bool extend) {
  const Camera &camera = canvas->GetCamera();
  ...
  Vector gu = camera.projUp.WithMagnitude(1), gr = camera.projRight.WithMagnitude(1);
  return DoLineTrimmedAgainstBox(canvas, hcs, ref, a, b, extend, gr, gu, swidth, sheight);
}

int Constraint::DoLineTrimmedAgainstBox(Canvas *canvas, Canvas::hStroke hcs, Vector ref, Vector a,
                                        Vector b, bool extend, Vector gr, Vector gu, double swidth,
                                        double sheight) {
  struct {
  ...
  planes[0].n = gu.ScaledBy(-1);
  planes[0].d = -(gu.Dot(ref) + sheight / 2);
  planes[1].n = gu;
  planes[1].d = gu.Dot(ref) - sheight / 2;
  planes[2].n = gr;
  planes[2].d = gr.Dot(ref) - swidth / 2;
  planes[3].n = gr.ScaledBy(-1);
  planes[3].d = -(gr.Dot(ref) + swidth / 2);
  ...
  for (int i = 0; i < 4; i++) {
    bool   parallel;
    Vector p = VectorAtIntersectionOfPlaneAndLine(planes[i].n, planes[i].d, a, b, &parallel);
    if (parallel)
      continue;
...
void Constraint::DoArrow(Canvas *canvas, Canvas::hStroke hcs, Vector p, Vector dir, Vector n,
...
// center of the label).
//-----------------------------------------------------------------------------
void Constraint::DoLineWithArrows(Canvas *canvas, Canvas::hStroke hcs, Vector ref, Vector a,
                                  Vector b, bool onlyOneExt) {
  const Camera &camera = canvas->GetCamera();
  double        pixels = 1.0 / camera.scale;
  ...
  if (within != 0) {
    ...
    if (within < 0)
      DoLine(canvas, hcs, ae, ae.Minus(seg));
    if (within > 0)
      DoLine(canvas, hcs, be, be.Plus(seg));
  }
...
void Constraint::DoEqualLenTicks(Canvas *canvas, Canvas::hStroke hcs, Vector a, Vector b, Vector gn,
                                 Vector *refp) {
  const Camera &camera = canvas->GetCamera();
...
void Constraint::DoEqualRadiusTicks(Canvas *canvas, Canvas::hStroke hcs, hEntity he, Vector *refp) {
  const Camera &camera = canvas->GetCamera();
...
void Constraint::DoArcForAngle(Canvas *canvas, Canvas::hStroke hcs, Vector a0, Vector da, Vector b0,
                               Vector db, Vector offset, Vector *ref, bool trim,
  ...
  VectorAtIntersectionOfLines_ret eeep =
      VectorAtIntersectionOfLines(a0, a0.Plus(da), b0, b0.Plus(db), true);
  ...
  if (!eeep.skewed) {
    *ref = pi.Plus(offset);
    ...
    // Calculate median
    Vector m  = da.ScaledBy(cos(thetaf / 2)).Plus(dna.ScaledBy(sin(thetaf / 2)));
    ...
    *ref           = pi.Plus(rm.WithMagnitude(std::max(rm.Magnitude(), 15 * pixels + textR)));
    ...
    Vector apb = da.ScaledBy(r * cos(thetaf)).Plus(dna.ScaledBy(r * sin(thetaf))).Plus(pi);
    ...
    for (int i = 0; i <= n; i++) {
      ...
      Vector p     = da.ScaledBy(r * cos(theta)).Plus(dna.ScaledBy(r * sin(theta))).Plus(pi);
      if (i > 0) {
        if (trim) {
    ...
    *ref              = (*ref).ScaledBy(0.5).Plus(disp.offset);
    ...
    double textHeight = Style::TextHeight(GetStyle()) / camera.scale;
    Vector trans = (*ref).Plus(gu.ScaledBy(-1.5 * VectorFont::Builtin()->GetCapHeight(textHeight)));
    canvas->DrawVectorText("angle between skew lines", textHeight, trans, gr.WithMagnitude(1),
                           gu.WithMagnitude(1), hcs);
  }
}
...
bool Constraint::DoLineExtend(Canvas *canvas, Canvas::hStroke hcs, Vector p0, Vector p1, Vector pt,
                              double salient) {
...
void Constraint::DoLayout(DrawAs how, Canvas *canvas, Vector *labelPos, std::vector<Vector> *refs) {
  ...
  case Type::PROJ_PT_DISTANCE: {
    Vector ap = SK.GetEntity(ptA)->PointGetNum(), bp = SK.GetEntity(ptB)->PointGetNum(),
           dp = (bp.Minus(ap)), pp = SK.GetEntity(entityA)->VectorGetNum();
  ...
  case Type::PT_LINE_DISTANCE: {
    ...
    bool shouldExplode = ShouldDrawExploded() && ptEntity->group == group && line->group == group;
    if (shouldExplode) {
  ...
  case Type::POINTS_COINCIDENT: {
    if (how == DrawAs::DEFAULT) {
      ...
      RgbaColor cd = Style::Color(Style::DATUM), cc = Style::Color(Style::CONSTRAINT);
    ...
    for (int a = 0; a < 2; a++) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        if (refs)
          refs->push_back(p);
        canvas->DrawQuad(p.Plus(r).Plus(d), p.Plus(r).Minus(d), p.Minus(r).Minus(d),
                         p.Minus(r).Plus(d), hcf);
      }
    }
    return;
  }
  ...
  case Type::WHERE_DRAGGED: {
    ...
    if (refs)
      refs->push_back(p);
    Vector u  = p.Plus(gu.WithMagnitude(8 / camera.scale)).Plus(gr.WithMagnitude(8 / camera.scale)),
  ...
  case Type::EQUAL_ANGLE: {
    ...
    DoArcForAngle(canvas, hcs, a0, da, b0, db, da.WithMagnitude(40 / camera.scale), &ref,
                  /*trim=*/false, a->ExplodeOffset());
    if (refs)
      refs->push_back(ref);
    DoArcForAngle(canvas, hcs, c0, dc, d0, dd, dc.WithMagnitude(40 / camera.scale), &ref,
                  /*trim=*/false, c->ExplodeOffset());
    if (refs)
      refs->push_back(ref);
  ...
  case Type::ARC_LINE_TANGENT: {
    ...
    if (type == Type::ARC_LINE_TANGENT) {
      ...
      Vector  p    = SK.GetEntity(arc->point[other ? 2 : 1])->PointGetDrawNum();
      Vector  r    = p.Minus(c);
      ...
      Vector  p     = other ? cubic->CubicGetFinishNum() : cubic->CubicGetStartNum();
      ...
      if (other) {
        ...
        if (eA->type == Entity::Type::CUBIC) {
          ...
          dir = SK.GetEntity(eA->point[0])
                    ->PointGetNum()
                    .Minus(SK.GetEntity(eA->point[2])->PointGetNum());
          dir = n.Cross(dir);
        }
      } else {
        ...
        if (eA->type == Entity::Type::CUBIC) {
          ...
          dir = SK.GetEntity(eA->point[0])
                    ->PointGetNum()
                    .Minus(SK.GetEntity(eA->point[1])->PointGetNum());
          dir = n.Cross(dir);
        }
      }
      dir    = n.Cross(dir);
    ...
    canvas->DrawVectorText("T", textHeight, textAt.Minus(ex.ScaledBy(0.5)), u.WithMagnitude(1),
                           v.WithMagnitude(1), hcs);
    if (refs)
      refs->push_back(textAt);
    return;
  }
  ...
  case Type::PARALLEL: {
    for (int i = 0; i < 2; i++) {
      ...
      if (refs)
        refs->push_back(p.Plus(n.ScaledBy(0.5)));
    }
    return;
  }
  ...
  case Type::EQUAL_LINE_ARC_LEN: {
    Entity *line = SK.GetEntity(entityA);
    Vector  ref  = Vector(0, 0, 0);
    DoEqualLenTicks(canvas, hcs, SK.GetEntity(line->point[0])->PointGetDrawNum(),
                    SK.GetEntity(line->point[1])->PointGetDrawNum(), gn, &ref);
    if (refs)
      refs->push_back(ref);
  ...
  case Type::EQ_PT_LN_DISTANCES: {
    ...
    {
    case Type::SYMMETRIC: n = SK.GetEntity(entityA)->Normal()->NormalN(); goto s;
    case Type::SYMMETRIC_HORIZ: n = SK.GetEntity(workplane)->Normal()->NormalU(); goto s;
    case Type::SYMMETRIC_VERT: n = SK.GetEntity(workplane)->Normal()->NormalV(); goto s;
  ...
  case Type::VERTICAL:
    if (entityA.v) {
      ...
      Vector shift = r.WithMagnitude(ex.x).Plus(u.WithMagnitude(ex.y));

      canvas->DrawVectorText(s, textHeight, o.Minus(shift.ScaledBy(0.5)), r.WithMagnitude(1),
                             u.WithMagnitude(1), hcs);
      if (refs)
        refs->push_back(o);
    } else {
      Vector a = SK.GetEntity(ptA)->PointGetDrawNum();
      ...
      for (i = 0; i < 2; i++) {
        ...
        canvas->DrawQuad((c.Plus(d)).Plus(dp), (c.Minus(d)).Plus(dp), (c.Minus(d)).Minus(dp),
                         (c.Plus(d)).Minus(dp), hcf);
        if (refs)
          refs->push_back(c);
      }
    }
